...
[Type generation](https://webdriver.io/docs/flowcharts/typegeneration) - Protocol specs → compiler → generated types → typings tests and API docs.
...
* Switch to the Node.js version in [`.nvmrc`](.nvmrc). WebdriverIO v10 requires Node.js 22.19.0 or later (22, 24, and 26). We recommend [`nvm`](https://github.com/nvm-sh/nvm) to switch between Node.js versions.
...
The protocol types are generated as part of the `@wdio/compiler` build command and is part of the [`generate-types`](https://github.com/webdriverio/webdriverio/blob/main/infra/compiler/src/type-generation/index.ts#L27-L35) plugin. The [type generation flowchart](https://webdriver.io/docs/flowcharts/typegeneration) shows the full path, including BiDi CDDL and `pnpm run test:typings`.
