...
describe('Lit Component testing', () => {
    ...
    it('does not stale on elements not found', async () => {
        ...
        await expect($('non-existing-element')).not.toBePresent( { wait: 0 })
        await expect($('non-existing-element')).not.toBeDisplayed({ wait: 0 })
        await expect(await $('non-existing-element')).not.toBePresent({ wait: 0 })
        await expect(await $('non-existing-element')).not.toBeDisplayed({ wait: 0 })
