...
import { downloadFromGitHub, buildPreface, getRootDir } from '@wdio/repo-utils'
...
export const docsFixes: Record<string, ((docs: string) => string)> = {
...
export interface ThirdPartyDocsOptions {
    rootDir?: string
}
 ...
 */
export async function generate3rdPartyDocs (sidebars: any, options: ThirdPartyDocsOptions = {}) {
 ...
 */
export function normalizeDoc(readme: string, githubUrl: string, branch: string, preface: string[], repoInfo: string[], packageName: string) {
    /**
     * remove badges
     */
 ...
 */
export function buildInfo(packageName: string, githubUrl: string, npmUrl: string) {
    return [
