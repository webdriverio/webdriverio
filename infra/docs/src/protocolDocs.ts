...
} from '@wdio/repo-utils/protocols'
...
export interface ProtocolDocsOptions {
    websiteDir?: string
}
 ...
 */
export function generateProtocolDocs (sidebars: any, options: ProtocolDocsOptions = {}) {
    const websiteDir = options.websiteDir ?? path.join(__dirname, '..', '..', '..', 'website')
    const apiDocsRootDir = path.join(websiteDir, 'docs', category)

    fs.mkdir(apiDocsRootDir, { recursive: true }, (err) => console.error(err))
    ...
    for (const [protocolName, definition] of Object.entries(PROTOCOLS)) {
        ...
        const docPath = path.join(apiDocsRootDir, `_${protocolName}.md`)
