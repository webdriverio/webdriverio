...
export interface WdioDocsOptions {
    rootDir?: string
}
 ...
 */
export async function generateWdioDocs (sidebars: any, options: WdioDocsOptions = {}) {
    ...
    for (const [scope, [id, files]] of Object.entries(COMMANDS)) {
        ...
        for (const file of files) {
            const docDir = path.join(rootDir, 'website', 'docs', 'api', scope)
            ...
            apiDocs[apiDocs.length - 1].items?.push(`api/${scope}/${file.replace(/\.(js|ts)/, '')}`)
        }
    }
