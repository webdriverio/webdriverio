...
let moveToObject = function (selector, xoffset = 0, yoffset = 0) {
    /**
    ...
    if (this.isMobile) {
        return this.element(selector).then((res) =>
            this.elementIdSize(res.value.ELEMENT).then((size) =>
                this.elementIdLocation(res.element.value.ELEMENT).then((location) => {
