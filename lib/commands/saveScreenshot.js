 ...
 * <example>
 ...
 client.saveScreenshot(function(err, screenshot) { ... });
 // save screenshot to file and receive as Buffer
 client.saveScreenshot('snapshot.png', function(err, screenshot) { ... });
 // save screenshot to file
 client.saveScreenshot('snapshot.png');
 * </example>
 *
 * @param {Function|String}   response    callback or path to generated image (relative to the execution directory)
 *
...
module.exports = function saveScreenshot(response) {
    ...
    async.waterfall([
        ...
        function(res, cb) {
            ...
            if (typeof response === 'string') {
                fs.writeFile(response, screenshot, cb);
            } else {
                cb();
            }
        }
    ], function(err) {
        callback(err, screenshot);
    });
