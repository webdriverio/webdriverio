    ...
    function setViewportSize(client, retryNo) {
        ...
        return client.windowHandleSize().then(function(windowHandleSize) {
            ...
            return this.execute(getViewportSizeHelper).then(function (viewportSize) {
                ...
                /**
                 * If the viewport's width does not equal the desired width,
                 * execute the process again.
                 *
                 * We care about the width, as it is not always possible to get
                 * the height correctly.
                 */
                if (retryNo < MAX_TRIES && res.value.screenWidth !== size.width) {
