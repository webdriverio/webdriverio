...
let setViewportSize = function (size, type) {
    ...
    return shouldIndent ? _setViewportSize(this, size) : this.windowHandleSize(size)
}
 ...
 */
let _setViewportSize = function (client, size, retryNo = 0) {
    /**
     * get window size
     */
    return client.windowHandleSize().then((windowHandleSize) => {
        /**
         * get viewport size
         */
        return this.execute(getViewportSize).then((viewportSize) => {
            ...
            if (retryNo < MAX_TRIES && (res.value.screenWidth !== size.width || res.value.screenHeight !== size.height)) {
                return _setViewportSize(client, size, ++retryNo)
            }
        })
    })
}
