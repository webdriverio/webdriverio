...
module.exports = function(fn, selectors, args, callback) {
    ...
    return ('return (' + executeClientSide + ')(' + strArgs.join(',') + ');').replace(/(\s{4}|\t)+/g, ' ');
};
...
var executeClientSide = function(fn, sArr, args) {
    ...
    var elements = args && sArgs.concat(args) || sArgs;
    if(elements.length === 0 || (elements.length === 1 && elements[0].length === 0)) {
        return new Error('NoSuchElement')
    }

    return fn.apply(window, elements);
};