...
class Launcher {
    ...
    async run () {
        ...
        /**
         * run onPrepare hook for services
         */
        await Promise.all(launcher.map((service) => {
        ...
        /**
         * run onComplete hook for services
         */
        await Promise.all(launcher.map((service) => {
            if (typeof service.onComplete === 'function') {
                return service.onComplete(exitCode)
            }
        }))
