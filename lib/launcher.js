...
var init = function(configFile, argv) {
    ...
    function runSpecs() {
        ...
        schedule.forEach(function(capability, capId) {
            var specFiles = capability.specs.length;
            specsLeft += specFiles;

            for(var i = 0; i < capability.availableInstances && i < specFiles; i++) {
                startInstance([capability.specs.pop()], capId);
                schedule[capId].availableInstances--;
                schedule[capId].runningInstances++;;
            }
            isRunning = isRunning || schedule[capId].runningInstances > 0;
        });
    ...
    function messageHandler(m) {
        if(m.event === 'runner:end') {
            schedule[m.capId].availableInstances++;
            schedule[m.capId].runningInstances--;
        }
