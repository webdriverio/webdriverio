...
class Launcher {
    ...
    async run () {
        ...
        /**
         * throw error if all capabilities don't have any specs to run
         */
        if (this.schedule.filter((s) => s.specs.length === 0).length === this.schedule.length) {
            throw new Error(`No specs have been defined! This could have the following reason:
- your 'specs' glob is not matching any files
- you want to run a specifc suite that is not defined in your config\n`)
        }
    ...
    getRunnerId (cid) {
        ...
        this.cid = String.fromCharCode(this.cid.charCodeAt(0) + 1)
        return cid + this.cid
    }
