    ...
    startInstance (specs, caps, cid, server) {
        ...
        if (debug) {
            for (let i in process.execArgv) {
                if (process.execArgv[i].startsWith("--debug-brk")) {
                    debugArgs.push('--inspect-brk');
                } else if (process.execArgv[i].startsWith("--inspect=")) {
                    debugArgs.push('--inspect=' + (process.debugPort + processNumber));
                }
            }
        }
        ...
        if (debug) {
            console.debug(`Please attach the inspector at port ${(process.debugPort + processNumber)}`);
        }
