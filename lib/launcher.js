...
class Launcher {
    ...
    initReporters () {
        ...
        for (let reporterType of config.reporters) {
            let Reporter
            if (typeof reporterType === 'function') {
                Reporter = reporterType
            } else if (typeof reporterType === 'string') {
                try {
                    Reporter = require(`wdio-${reporterType}-reporter`)
                } catch (e) {
                    throw new Error(`reporter "wdio-${reporterType}-reporter" is not installed. Error: ${e.stack}`)
                }
            }
            if (!Reporter) {
                throw new Error(`reporter option must be either an array of strings or functions, but got: ${reporterType}`)
            }

            const reporterOptions = {}
            for (let option of Object.keys(config.reporterOptions)) {
                if (option === reporterType && typeof config.reporterOptions[reporterType] === 'object') {
                    // Copy over options specifically for this reporter type
                    for (let reporterOption of Object.keys(config.reporterOptions[reporterType])) {
                        reporterOptions[reporterOption] = config.reporterOptions[reporterType][reporterOption]
                    }
                } else if (config.reporters.indexOf(option) >= 0) {
