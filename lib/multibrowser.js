...
function Multibrowser(options) {
    ...
    Object.keys(options).forEach(function(instanceName) {
        var instance = new Instance(instanceName, options[instanceName], self);
...
Object.keys(WebdriverIOPrototype.prototype).forEach(function(fnName) {
    ...
    if(!Multibrowser.prototype[fnName] && nonSeleniumCommands.indexOf(fnName) > -1) {
        Multibrowser.prototype[fnName] = WebdriverIOPrototype.prototype[fnName];
        return;
    }
...
Multibrowser.prototype.instanceCommand = function(callingInstance, processMessage) {
    ...
    processMessage.commandId = this.commandState.length - 1;
    this.instances[callingInstance].cp.send(processMessage);
    this.eventHandler.on('instance_' + (this.commandState.length - 1), finishCommand.bind(this, callback));
};
...
var finishCommand = function(callback, instance, message){
    ...
    if(message.origin === this.origin) {
        ...
        commandResult.error = message.args[0];
        commandResult.result = message.args[1];
        commandResult.response = message.args[2];
    }

    /**
     * only execute callback and finish command once all instances have finished this command
     */
    if(message.origin !== 'instance' && Object.keys(commandResult.result).length !== Object.keys(this.instances).length) {
        return;
    }
     ...
     */
    if(commandResult.error && Object.keys(commandResult.error).length === 0) {
