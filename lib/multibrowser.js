...
function Multibrowser(options) {
    ...
    Object.keys(options).forEach(function(instanceName) {
        ...
        /**
         * register instance commands to chain them
         */
        instance.eventHandler.on('instance.command', function(instanceCommandId) {
            var commandId = self.commandState.length;

            self.commandState.push({
                state: self.fullmask ^ instance.flag
            });

            instance.eventHandler.once('instance.' + instanceCommandId, function(message) {
                /**
                 * lead multibrowser believe that command was executed by its self
                 * Note: onMessage gets triggered automatically
                 */
                message.origin = 'multibrowser';
    ...
    this.fullmask = parseInt(Array(Object.keys(this.instances).length + 1).join('1'), 2);
    ...
    this.flushCommandQueue = function(afterCommandId) {
        var self = this;

        while(this.commandQueue.length && this.commandQueue[0].message.commandId >= afterCommandId) {
     ...
     * by checking if bitmask of previous commands only contains 1's
     */
    this.instancesAreSynced = function(commandId) {
        for(var i = 0; i <= commandId; ++i) {
            if(this.fullmask !== this.commandState[i].state) {
                return false;
            }
        }
        return true;
...
Multibrowser.register = function(fnName, chainedCommand) {
    ...
    if(fnName === 'call') {
        return function(instances, callback) {
            ...
            this.eventHandler.on('multibrowser.*', function(finishedInstance, message) {
                /**
                 * check if instances are synced
                 */
                if(self.instancesAreSynced(commandId) && message.commandId === commandId) {
                    /**
                     * start instances again
                     */
                    self.startInstances(instances);
                    self.flushCommandQueue(commandId);
    ...
    return function() {
        ...
        this.eventHandler.on(origin + '.' + (this.commandState.length - 1), function(instance, message) {
...
var onMessage = function(instance, message) {
    /**
     * only listen on responses send by multibrowser
     */
    if(message.origin !== 'multibrowser') {
        return;
    }

    this.commandState[message.commandId].state |= instance.flag;
    this.eventHandler.emit(message.origin + '.' + message.commandId, instance, message);
};
