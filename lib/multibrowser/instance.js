...
function Instance(instanceName, capabilities) {
    ...
    this.onMessage = function(message) {
        this.eventHandler.emit(message.origin + '.' + message.commandId, message);
    };
...
Object.keys(webdriverio.prototype).forEach(function(fnName) {
    ...
    Instance.prototype[fnName] = function(){
        ...
        this.eventHandler.emit('instance.command', this.commandId, fnName);

        if(callback) {
            this.eventHandler.once(origin + '.' + this.commandId, function(message) {
