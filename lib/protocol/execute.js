...
import { getArgs } from '../scripts/createSelectorScript'
...
let execute = function (...args) {
    ...
    if (this.isIOS) {
        const strArgs = getArgs(args)
        script = typeof script === 'string' ? script : `(${script}).apply(null, ${strArgs})`
     ...
     */
    } else if (typeof script === 'function' || (this.inMultibrowserMode && script.indexOf('function (') === 0)) {
