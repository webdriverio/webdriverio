...
export default function session (doWhat = 'GET', sessionId) {
    ...
    if (doWhat === 'DELETE') {
        ...
        return this.requestHandler.create({
            ...
            if (this.options.isWDIOSessionCleanUp) {
                /**
                 * At the end of runner - mute any request-related error here (session not exist, connection refused, etc ...)
                 * Done in order to not break the runner, when selenium-part is broken for some reason.
                 */
                this.requestHandler.sessionID = null
                return {
                    value: [],
