...
function Base() {
    ...
    this.on('runner:init', function(runner) {
        stats.runner[runner.cid].sessionID = runner.sessionID;
    });
    ...
    this.on('test:pass', function(test) {
        stats.runner[test.cid].tests.push(null);
        stats.passes++;
    });

    this.on('test:fail', function(test) {
        stats.failures++;
        stats.runner[test.cid].tests.push(test.err);
    ...
    this.on('runner:end', function(runner) {
        stats.runner[runner.cid].end = new Date();
    });
