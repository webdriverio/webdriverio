...
var util = require('util'),
    ...
    fs = require('fs'),
    path = require('path'),
    sanitizeCaps = require('../helpers/sanitizeCaps');
...
function XUnit(options) {
    ...
    this.errorLogCharacterLimitation = 1000;
    this.indents = 0;
    this.errors = 0;
...
XUnit.prototype.genTestsuite = function(tests) {
    ...
    tests.forEach(function(test) {
        ...
        self.printLogTags(test.runnerData.result, 'system-out', 'result', function(data) {
            return (data.requestOptions.method || 'get').toUpperCase() + ' ' + data.requestOptions.uri.href + ' - ' + JSON.stringify(data.body) + '\n';
        });
        self.printLogTags(test.runnerData.error, 'system-err', 'runnererror', function(data) {
            return (data.requestOptions.method || 'get').toUpperCase() + ' ' + data.requestOptions.uri.href + '\n' +
                   'ERROR:' + data.err + '\n' +
                   'BODY:' + (typeof data.body === 'object' ? JSON.stringify(data.body) : data.body).slice(0, self.errorLogCharacterLimitation) + '\n';
        });
