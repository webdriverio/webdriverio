...
const WATCH_NOTIFICATION = '\nWDIO is no in watch mode and is waiting for a change...'
...
class Runner {
    ...
    runWatchMode (cid, config, specs) {
        const gaze = new Gaze(specs)

        console.log(WATCH_NOTIFICATION)
        gaze.on('changed', async (filepath) => {
            if (this.isRunning) {
            ...
            setTimeout(() => {
                ...
                if (this.fileTriggeredWhileRunning) {
                    gaze.emit('changed', this.fileTriggeredWhileRunning)
                    this.fileTriggeredWhileRunning = null
                }
            }, 500)
        })
