...
process.on('message', (m) => {
    switch(m.command) {
        case 'run':
            ...
            let capabilities = configParser.getCapabilities(m.cid)
            ...
            if(m.isMultiremote) {
                ...
                for (let [browserCaps, browserName] of options) {
                 ...
                 */
                for (let browserName in options) {
            ...
            if(typeof config.plugins === 'object') {
                for (let [pluginConfig, pluginName] of config.plugins) {
                    try {
            ...
            global.browser.init().then(() => {
                ...
                return new Promise.resolve(config.after(failures, capabilities))
            }).finally(() => {
                process.send({
                    event: 'runner:end',
                    failures: failures,
