...
class Runner {
    ...
    saveScreenshot (screenshot) {
         ...
         */
        if (typeof global.browser.options.screenshotPath !== 'string') {
            return
        }

        let screenshotPath = path.join(process.cwd(), global.browser.options.screenshotPath)

        /**
         * take screenshot only if directory exists
         */
        if (!fs.existsSync(screenshotPath)) {
            throw Error(`screenshotPath "${screenshotPath}" is not writeable`)
        }

        let filename = path.join(screenshotPath, 'ERROR_' + sanitize.caps(global.browser.desiredCapabilities) + '_' + new Date().toJSON() + '.png')
        fs.writeFileSync(filename, new Buffer(screenshot, 'base64'))

        global.browser.emit('screenshot', {data: screenshot, filename})
    }
    ...
    addCommandHooks (config) {
        ...
        config.afterCommand.push((command, args, result, err) => {
            if (err && err.seleniumStack && err.seleniumStack.screenshot) {
                this.saveScreenshot(err.seleniumStack.screenshot)
            }
