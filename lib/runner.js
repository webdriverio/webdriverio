...
class Runner {
    ...
    async run (m) {
        ...
        global.browser.on('command', (payload) => {
            ...
            if (currentTest) {
                command.title = currentTest.title
                command.parent = currentTest.parent
            }
        ...
        global.browser.on('result', (payload) => {
            ...
            if (currentTest) {
                result.title = currentTest.title
                result.parent = currentTest.parent
            }
            process.send(result)
        })

        global.browser.on('screenshot', (path) => {
            process.send({
    ...
    initialiseFramework (config) {
        ...
        try {
            ...
            throw new Error(
                ...
                `it with \`$ npm install wdio-${frameworkLibrary}-framework\`! Error: ${e.stack}`
            )
        }
    }
        ...
        for (let pluginName of Object.keys(config.plugins)) {
            ...
            try {
                ...
                throw new Error(
                    ...
                    `with \`$ npm install ${pluginName}\`! Error: ${e.stack}`
                )
            }
