...
class BaseReporter extends events.EventEmitter {
    constructor () {
        ...
        this.on('runner:result', (result) => {
            ...
            testStats.output.push(result)
            this.propagateEvent('runner:result', result)
        })

        this.on('runner:screenshot', (test) => {
            const testStats = this.getTestStats(test)
            testStats.screenshots.push(test.path)
            this.propagateEvent('runner:screenshot', test)
        })
    ...
    getSpecStats (obj) {
        ...
        if (!this.stats.runner[obj.cid].specs[obj.specHash]) {
    ...
    getSuiteStats (suite) {
        if (!suite.title) console.log('NO SUITE TITLE', suite, Error().stack)
        const specStats = this.getSpecStats(suite)
        if (!specStats.suites[suite.title]) {
    ...
    getTestStats (test) {
        ...
        if (!suiteStats.tests[test.title]) {
