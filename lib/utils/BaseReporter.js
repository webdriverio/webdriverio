...
class BaseReporter extends events.EventEmitter {
    constructor () {
        ...
        this.on('test:fail', (test) => {
            ...
            for (let failure in this.failures) {
        ...
        this.on('error', (m) => {
            ...
            var sanitizedCaps = []
            for (let [value, capability] in m.capabilities) {
                /**
                ...
                sanitizedCaps.push(capability + ': ' + JSON.stringify(value))
            }
    ...
    listFailures () {
        ...
        for (let [test, i] in this.failures) {
            let runningBrowser = ''
            for (let [caps, pid] in test.runner) {
                runningBrowser += '\nrunning'
