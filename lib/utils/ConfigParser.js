...
class ConfigParser {
     ...
     * @param {String | Object} filename path of file relative to current directory or the configuration object
     */
    addConfigFile (configFile) {
        if (typeof configFile === 'string') {
            /**
             * Configuration file is received as a string, load the file to get the configuration
            */
            var filePath = path.resolve(process.cwd(), configFile)
            if (!fs.existsSync(filePath)) {
                throw new Error(`Failed loading configuration file: ${filePath}`)
            }
            configFile = require(filePath).config
        } else if (configFile === null || typeof configFile !== 'object') {
            throw new Error('addConfigFile requires configuration')
        }
        ...
        var config
        try {
            config = merge(configFile, {}, MERGE_OPTIONS)
        ...
        try {
            ...
            this._capabilities = merge(this._capabilities, config.capabilities || defaultTo, MERGE_OPTIONS)
            ...
            this._config = merge(this._config, config, MERGE_OPTIONS)
            ...
            console.error('Failed loading configuration')
            throw e
        }
    }
