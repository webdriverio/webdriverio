...
class ConfigParser {
    ...
    merge (object = {}) {
         ...
         */
        if (this.isTypeOfString(object.specs)) {
            this._config.specs = [...this.setFilePathToFilterOptions(object.specs)]    
        } else if (this.isTypeOfString(object.exclude)) {
            this._config.exclude = [...this.setFilePathToFilterOptions(object.exclude, true)]
        }
    ...
    setFilePathToFilterOptions (filterObj, exclude = false) {
        ...
        if (objs.size === 0) {
            throw new Error(`${FILTER_TYPE} file ${filterObj} not found`)
        }
