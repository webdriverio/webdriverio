...
class ConfigParser {
     ...
     */
    addConfigFile (argConfig) {
        try {
            /**
            * reads config from either a file or an config object
            */
            var fileConfig = {}
            if (typeof argConfig === 'string') {
                var filePath = path.resolve(process.cwd(), argConfig)
                fileConfig = merge(require(filePath).config, {})
            } else if (typeof argConfig === 'object') {
                fileConfig = merge(argConfig, {})
            } else {
                throw new Error('addConfigFile needs to be a filepath or an configuration object')
            }
