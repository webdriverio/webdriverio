...
const HOOKS = ['onPrepare', 'before', 'beforeSuite', 'beforeHook', 'beforeTest', 'beforeCommand',
               'afterCommand', 'afterTest', 'afterHook', 'afterSuite', 'after', 'onComplete']
    ...
    addService (service) {
        for (let hookName of HOOKS) {
            if (!service[hookName]) {
                ...
                this._config[hookName].push(service[hookName])
            }
        }
    }
