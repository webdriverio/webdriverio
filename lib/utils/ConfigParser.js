...
class ConfigParser {
    ...
    merge (object = {}) {
         ...
         */
        if (this.isTypeOfString(object.spec)) {
            this._config.specs = [...this.setFilePathToFilterOptions(object.spec, this._config.specs)]
        }

        if (this.isTypeOfString(object.exclude)) {
            this._config.exclude = [...this.setFilePathToFilterOptions(object.exclude, this._config.exclude)]
        }
     ...
     */
    setFilePathToFilterOptions (filterObj, config) {
        const objs = new Set()
        const objList = filterObj.split(/,/g)
        const objsList = ConfigParser.getFilePaths(config)

        for (let obj of objList) {
            if (fs.existsSync(obj) && fs.lstatSync(obj).isFile()) {
                objs.add(path.resolve(process.cwd(), obj))
                continue
            }

            const paramFiles = objsList.find(file => file.match(obj))
            if (paramFiles) {
                objs.add(paramFiles)
            }
        }
        ...
        if (objs.size === 0) {
            throw new Error(`File ${filterObj} not found`)
        }
