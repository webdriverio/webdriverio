...
class Logger {
    constructor (options, eventHandler) {
         ...
         */
        if (options.logLevel != 'silent' && options.logLevel != 'command' && options.logLevel != 'result' && options.logLevel != 'error' && options.logLevel != 'verbose') {
        ...
        eventHandler.on('result', (data) => {
            ...
            if (data.body && data.body.value && (this.logLevel === 'result' || this.logLevel === 'verbose')) {
                this.result(data.body.value ? data.body.value : data.body.orgStatusMessage)
            }
        })
