...
module.exports.outer = function PromiseHandlerInner(fnName, fn) {
    return function() {
        ...
        var thenMethod = function(onFulfilled, onRejected) {
            ...
            var resolvePromise = function(promise) {
                ...
                if(returnPromise && returnPromise.then) {
                    lastThenable = returnPromise.getPromise();
                }

            };

            self.call(function() {
                var promise = lastThenable || deferred.promise;
        ...
        this.then = this.catch = this.fail = this.reject = thenMethod
