...
class RequestHandler {
    ...
    create (requestOptions, data) {
        ...
        return this.requestWithRetry(fullRequestOptions, this.defaultOptions.connectionRetryCount).then(({body, response}) => {
            /**
    ...
    requestWithRetry (fullRequestOptions, totalRetryCount) {
        return new Promise((resolve, reject) => {
            let retry = (count) => {
                this.request(fullRequestOptions).then(resolve).catch((error) => {
                    if (++count === totalRetryCount) {
                        this.eventHandler.emit('info', 'Gave up after ' + totalRetryCount + ' tries')
                        return reject(error)
                    }

                    retry(count)

                    this.eventHandler.emit('info', 'Retry #' + count)
