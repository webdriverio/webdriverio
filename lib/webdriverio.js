        ...
        client.next = function (func, args, name) {
            ...
            return this.lastPromise.then((val) => {
                ...
                if (hasElementResult(lastResult) && args.length < func.length && func.toString().indexOf(`function ${name}(selector`) === 0) {
                    const isWaitForWithSelector = name.match(/^waitFor/) && typeof args[0] === 'string'
                     ...
                     * selected
                     */
                    if (lastResult.state === 'failure') {
                        const message = lastResult.message.replace(/^An element/, `An element ("${lastResult.selector}")`)
                        const error = new Error(message)
                        return resolve.call(this, error, null, null, { depth: 0 })
                    }

                    if (lastResult.selector && !isWaitForWithSelector) {
