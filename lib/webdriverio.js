...
module.exports = function WebdriverIO(args, modifier) {
    ...
    // this will help us if saveScreenshot will fail
    var previousError;
    ...
    var resolve = function(result, isErrorHandled) {
         ...
         */
        return unit().next(prototype.saveScreenshot, [
            path.join(getScreenshotPath(), screenshotName)
        ], 'saveScreenshot').finally(function() {
            previousError = null;
            logger.info('\t' + screenshotName);

            this.defer.reject(result);
            throwException(result, stacktrace);
        }.bind(this));

    };

    function canSaveScreenshot() {
        /**
        ...
        return fs.existsSync(getScreenshotPath());

    }

    function getScreenshotPath() {
        return path.join(process.cwd(), options.screenshotPath);
    }
