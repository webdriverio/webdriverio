...
module.exports = function WebdriverIO(args, modifier) {
    ...
    var resolve = function(result, hasExceptionQueue, isRejectionNotHandled) {
         ...
         */
        if(isRejectionNotHandled !== false && (hasExceptionQueue || (this.promise.isRejected() && this.lastPromise.isRejected()))) {
            /**
             * propagate error from inside out
             */
            if(!this.gotCalledFromOutside) {
                /**
                 * Instead of throwing an error, we could gracefully end the Selenium session here
                 * and then throw the error. Since the developer is responsible for taking care of
                 * that, we'll leave it as it is.
                 */
                throwOnNextTick(result);
            }
    ...
    function throwOnNextTick(e) {
        return process.nextTick(function() {
            logger.printException(e.type, e.message, stacktrace);
            process.exit(1);
        });
    }
