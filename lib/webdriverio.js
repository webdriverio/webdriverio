...
let WebdriverIO = function (args, modifier) {
    ...
    let fulFilledPromise = Promise.resolve()
    let stacktrace = []
    let commandList = []
    ...
    function unit (lastPromise) {
        ...
        client.then = function (onFulfilled, onRejected) {
             ...
             */
            let client = unit(this.promise.then(() => {
                return resolve.call(client, safeExecute(onFulfilled, arguments).bind(this))
            }, () => {
                return resolve.call(client, safeExecute(onRejected, arguments).bind(this), typeof onRejected === 'function')
            }))
     ...
     */
    for (let eventCommand of EVENTHANDLER_FUNCIONS) {
