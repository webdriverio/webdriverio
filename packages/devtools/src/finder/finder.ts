...
export const darwinGetAppPaths = (app: string) => {
    const apps: ApplicationDataType = JSON.parse(
        execSync(DARWIN_LIST_APPS).toString(),
    )
    const appPaths = apps.SPApplicationsDataType.filter(
        (inst) => inst.info && inst.info.startsWith(app),
    ).map((inst) => inst.path)

    return appPaths
}

export const darwinGetInstallations = (
    appPaths: string[],
    suffixes: string[],
) => {
