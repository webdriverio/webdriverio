...
export async function findElement(
    ...
    const waitForFn =
        using === 'xpath'
            ? (context as Page | Frame).waitForXPath
            : (context as Page | Frame).waitForSelector
    if (implicitTimeout) {
...
export function sanitizeError(err: Error) {
    ...
    err.stack =
        ...
        stack
            ...
            .filter(
                (line) =>
                    !line.includes('devtools/node_modules/puppeteer-core'),
            )
            .join('\n')
    return err
}
 ...
 */
export async function transformExecuteArgs(
    this: DevToolsDriver,
    args: any[] = [],
...
export async function transformExecuteResult(
    ...
    if (
        tmpResult.find(
            (r: any) => typeof r === 'string' && r.startsWith(SERIALIZE_FLAG),
        )
    ) {
        tmpResult = await Promise.all(
            tmpResult.map(async (r: any) => {
                if (typeof r === 'string' && r.startsWith(SERIALIZE_FLAG)) {
                    return findElement.call(
                        this,
                        page,
                        'css selector',
                        `[${SERIALIZE_PROPERTY}="${r}"]`,
                    )
                }
...
export async function patchDebug(scoppedLogger: Logger) {
    ...
    try {
        ...
        debug.log = (msg: string) => {
            if (msg.includes('puppeteer:protocol')) {
                msg = msg
                    ...
                    .map((l) =>
                        l
                            .slice(Math.max(l.indexOf('◀ '), l.indexOf('► ')))
                            .replace('\x1B[32m', '')
                            .replace('\x1B[39m', '')
                            .replace('◀ \x1B[0m', '')
                            .replace('► \x1B[0m', '')
        ...
        log.warn(
            "Couldn't stub Puppeteer debug package, Puppeteer logs might get lost",
        )
    }
}
...
export const launchChromeUsingWhich = async (
    err: Error,
    ...
    const chromePath = await which('chrome').catch((errorChrome: Error) =>
        which('chromium').catch((errorChromium: Error) =>
            which('google-chrome').catch((errorGoogleChrome: Error) => {
                throw new Error(
                    util.format(
                        errorMessage,
                        errorChrome.message,
