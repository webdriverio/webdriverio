...
const setNormalPageLoadBehavior = () =>
    executionContext.evaluate.mockImplementation(() =>
        Promise.resolve(++evaluateCommandCalls % 2 === 1 ? '1' : 'complete'),
    )

const frame = {
    executionContext: vi
        .fn()
        .mockImplementation(() => Promise.resolve(executionContext)),
}
...
test('should return proper result', async () => {
    driver.commands.elementClick = (...args: any[]) =>
        new Promise<any>((resolve) =>
            setTimeout(() => resolve.apply(null, args), 100),
        )
    ...
    const errorMsg2 = await command
        .call(emit as any, '123')
        .catch((e) => e.message)
    await expect(errorMsg2).toContain('Wrong parameters applied')

    emit.mockReset()
    const errorMsg3 = await command.call({ emit } as any, '123', 'some text', [
        'some value',
    ])
...
test('should throw if command throws', async () => {
    driver.commands.elementClick = () =>
        new Promise((resolve, reject) =>
            setTimeout(() => reject(new Error('foobar')), 100),
        )

    const command = driver.register(commandMock)

    const emit = vi.fn()
    const errorMsg3 = await command
        .call({ emit } as any, '123', 'some text', ['some value'])
        .catch((e) => e.message)
...
test('should rerun command if it was executed within navigation', async () => {
            ...
            new Promise((resolve, reject) =>
                setTimeout(() => {
                    if (!wasCommandCalled) {
                        ...
                        reject(
                            new Error(
                                'foobar most likely because of a navigation',
                            ),
                        )
                    }
    ...
    const emit = vi.fn()
    const result = await command.call({ emit } as any, '123', 'some text', [
        'some value',
    ])
...
test('should throw if execution context can not be established', async () => {
    ...
    executionContext.evaluate = vi
        ...
        .mockImplementation(
            () =>
                new Promise((resolve, reject) =>
                    setTimeout(() => reject(new Error('ups')), 100),
                ),
        )
...
test('framenavigatedHandler for main frame', () => {
    ...
    const frameMock = {
        url: vi.fn().mockReturnValue('foobar'),
...
test('_targetCreatedHandler', async () => {
    ...
    await driver['_targetCreatedHandler']({
        page: vi.fn().mockReturnValue(Promise.resolve(null)),
    } as any)
    expect(driver.windows.size).toBe(1)
    await driver['_targetCreatedHandler']({
        page: vi.fn().mockReturnValue(Promise.resolve('foobar')),
    } as any)
