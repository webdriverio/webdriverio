...
import {
    ...
    getPages,
    getPrototype,
    getStaleElementError,
    launchChromeUsingWhich,
    patchDebug,
    sanitizeError,
    sleep,
    transformExecuteArgs,
    transformExecuteResult,
    uniq,
    validate,
} from '../src/utils.js'
...
describe('validate', () => {
    ...
    it('should do a proper request', () => {
        const body = validate(
            ...
            command.variables,
            command.ref,
            ['123', 'css selector', '#body', undefined],
        )
...
test('getPrototype', () => {
    ...
    const filteredCommands = Object.entries(commands).reduce(
...
describe('findElement utils', () => {
    ...
    describe('findElement', () => {
        it('tries to find element using css selector', async () => {
            ...
            expect(
                await findElement.call(
                    scope as any,
                    pageMock as any as any,
                    'css selector',
                    'barfoo',
                ),
        ...
        it('should fail if not found', async () => {
            ...
            const errorMessage = await findElement.call(
                scope as any,
                pageMock as any,
                'css selector',
                'barfoo',
            )
            expect((errorMessage as Error).message).toContain(
                'Element with selector "barfoo" not found',
            )
        })
        ...
        it('should not fail with the same error if Puppeteer can not find the element', async () => {
            ...
            pageMock.$.mockReturnValue(
                Promise.reject(new Error('failed to find element')),
            )
...
describe('sanitizeError', () => {
    it('should set stale element error', () => {
        ...
        expect(sanitizeError(err).stack).toContain(
            'stale element reference: The element reference is stale',
        )
    })
    ...
    it('should not contain puppeteer traces', () => {
        ...
        expect(sanitizeError(err).stack).not.toContain(
            'devtools/node_modules/puppeteer-core',
        )
    })
})
...
test('transformExecuteArgs', async () => {
    ...
    expect(
        await transformExecuteArgs.call(scope as any, [
            'foo',
...
test('transformExecuteArgs throws stale element if element is not in store', async () => {
    ...
    expect(
        transformExecuteArgs.call(scope as any, [
            'foo',
...
describe('transformExecuteResult', () => {
    test('multiple results', async () => {
        ...
        pageMock.$.mockReturnValue(Promise.resolve(42))
        expect(
            await transformExecuteResult.call(scope as any, pageMock as any, [
                'foobar',
                '__executeElement',
                42,
            ]),
    ...
    test('single result', async () => {
        ...
        pageMock.$.mockReturnValue(Promise.resolve(42))
        expect(
            await transformExecuteResult.call(
                scope as any,
                pageMock as any,
                'foobar',
            ),
...
test('findByWhich', () => {
    ...
    expect(
        findByWhich(['firefox'], [{ regex: /firefox/, weight: 51 }]),
    ).toEqual([])
...
describe('launchChromeUsingWhich', () => {
    it('should throw if error is not related to chrome binary not found', async () => {
        await expect(() =>
            launchChromeUsingWhich(new Error('ups'), {}),
        ).rejects.toThrow('ups')
    })
