...
import { commandEnd, commandStart } from './__fixtures__/command.js'
...
describe('reporter option "useCucumberStepReporter" set to true', () => {
        ...
        describe('Passing tests', () => {
            ...
            beforeAll(() => {
                ...
                const suiteResults: any = {
                    tests: [cucumberHelper.testPass()],
                    hooks: new Array(2).fill(cucumberHelper.hookEnd()),
                }
                ...
                allureXml = results.find(
                    (xml: any) => xml('ns2\\:test-suite').length >= 1,
                )
            })
            ...
            it('should not report passing hook', () => {
                expect(allureXml('step > name').eq(0).text()).not.toContain(
                    'Hook',
                )
                expect(allureXml('step > title').eq(0).text()).not.toContain(
                    'Hook',
                )
            })
            ...
            it('should detect analytics labels in test case', () => {
                expect(
                    allureXml('test-case label[name="feature"]')
                        .eq(0)
                        .attr('value'),
                ).toEqual('MyFeature')
                expect(
                    allureXml('test-case label[name="language"]')
                        .eq(0)
                        .attr('value'),
                ).toEqual('javascript')
                expect(
                    allureXml('test-case label[name="framework"]')
                        .eq(0)
                        .attr('value'),
                ).toEqual('wdio')
            })
            ...
            it('should add browser name as test argument', () => {
                expect(
                    allureXml('test-case parameter[kind="argument"]'),
                ).toHaveLength(1)
                expect(
            ...
            it('should detect tags labels on top in test case', () => {
                expect(
                    allureXml('test-case label[name="severity"]')
                        .eq(0)
                        .attr('value'),
                ).toEqual('critical')
            })
            ...
            it('should detect description on top in test case', () => {
                expect(
                    allureXml('test-case > description').eq(0).text(),
                ).toEqual('My scenario description')
                expect(
                    allureXml('test-case description[type="text"]'),
                ).toHaveLength(1)
            })

            it('should move attachments from successful hook to test-case', () => {
                expect(
                    allureXml('test-case > attachments > attachment').length,
                ).toEqual(1)
            })
        })
    })
    ...
    describe('Passing tests', () => {
        ...
        it('should have the console log add', () => {
            expect(
                allureXml(
                    'test-case > steps > step > attachments > attachment',
                ),
            ).toHaveLength(1)
            expect(
                allureXml('test-case > steps > step > attachments > attachment')
                    .eq(0)
                    .attr('title'),
            ).toBe('Console Logs')
        })
        ...
        describe('steps', () => {
            ...
            it('should add step from command', () => {
                expect(allureXml('step > name').eq(1).text()).toEqual(
                    'GET /session/:sessionId/element',
                )
                expect(allureXml('step > title').eq(1).text()).toEqual(
                    'GET /session/:sessionId/element',
                )
                expect(
        ...
        it('should detect description on top in test case', () => {
            expect(allureXml('test-case > description').eq(0).text()).toEqual(
    ...
    describe('Failed tests', () => {
        ...
        it('should handle failed hook', () => {
            ...
            reporter.onTestSkip(cucumberHelper.testSkipped())
            const suiteResults: any = {
                tests: [cucumberHelper.testSkipped()],
                hooks: [cucumberHelper.hookFail()],
            }
            reporter.onSuiteEnd(cucumberHelper.scenarioEnd(suiteResults))
            reporter.onSuiteEnd(cucumberHelper.featureEnd(suiteResults))
            reporter.onRunnerEnd(runnerEnd())
