...
describe('reporter option "useCucumberStepReporter" set to true', () => {
    ...
    describe('reporter option "disableWebdriverStepsReporting" set to true', () => {
        describe('Passing tests', () => {
            ...
            it('should convert tag label "issue" to allure link', () => {
                ...
                expect(issueLinks[0].url).toEqual(
                    'https://github.com/webdriverio/webdriverio/issues/BUG-987',
                )
            })
            ...
            it('should detect description on top in test case', () => {
                expect(allureResult.description).toEqual(
    ...
    describe('Passing tests', () => {
        ...
        beforeAll(() => {
            ...
            reporter.onSuiteStart(
                cucumberHelper.scenarioStart(
                    'my-awesome-feature-at-scenario-level',
                ),
            )
            reporter.onHookStart(cucumberHelper.hookStart())
            reporter.onHookEnd(cucumberHelper.hookEnd())
        ...
        it('should have the console log add', () => {
            ...
            expect(allureResult.steps[0].attachments[0].name).toEqual(
                'Console Logs',
            )
        })
        ...
        describe('steps', () => {
            ...
            beforeEach(() => {
                nonHookSteps = allureResult.steps.filter(
                    (step: StepResult) => step.name !== 'Hook',
                )
            ...
            it('should add step from command for test', () => {
                ...
                expect(nonHookSteps[0].steps[0].name).toEqual(
            ...
            expect(features).toEqual(
                expect.arrayContaining([
                    { name: LabelName.FEATURE, value: 'MyFeature' },
                    {
                        ...
                        value: 'my-awesome-feature-at-scenario-level',
                    },
                ]),
            )
    ...
    describe('Skipped test', () => {
        ...
        beforeAll(() => {
            ...
            reporter.onRunnerStart(runnerStart())
            reporter.onSuiteStart(
                cucumberHelper.featureStart(
                    'my-awesome-feature-at-feature-level',
                ),
            )
            reporter.onSuiteStart(cucumberHelper.scenarioStart())
            reporter.onTestStart(cucumberHelper.testStart())
        ...
        it('should have the console log add', () => {
            expect(allureResult.steps[0].attachments).toHaveLength(1)
            expect(allureResult.steps[0].attachments[0].name).toBe(
                'Console Logs',
            )
        })
    })
    ...
    describe('Skipped test after several steps passed', () => {
        ...
        it('should report one passed step', () => {
            const passedSteps = allureResult.steps.filter(
                (step) => step.status === Status.PASSED,
            )
        ...
        it('should report one canceled step', () => {
            const skippedSteps = allureResult.steps.filter(
                (step) => step.status === Status.SKIPPED,
            )
    ...
    describe('Failed tests', () => {
        ...
        it('should handle failed test', () => {
            ...
            const suiteResults: any = {
                tests: [
                    { state: 'failed', error: { message: 'assertionerror' } },
                ],
            }
            ...
            const browserParameter = allureResult.parameters.find(
                ...
                tests: [
                    {
                        ...
                        error: {
                            message:
                                'element ("mwc-checkbox") still not existing after 10000ms',
                        },
                    },
                ],
            }
    ...
    describe('Data Table', () => {
        ...
        it('should add data table as attachment to test-case', () => {
            const testCaseStep = allureResult.steps.find(
                (step) => step.name !== 'Hook',
            )
    ...
    describe('Hooks removal', () => {
        ...
        it('should remove empty hook with no steps or files attached', () => {
            ...
            const hookStep = allureResult.steps.find(
                (step: { name: string }) => step.name === 'Hook',
            )
