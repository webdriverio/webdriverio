...
import { getErrorTemplate, getTemplate } from '../utils.js'
...
const protocolCommandList = Object.values(commands)
    .map((endpoint) => Object.values(endpoint).map(({ command }) => command))
    .flat()
...
export function testrunner(options: WebdriverIO.BrowserRunnerOptions): Plugin {
    const automationProtocolPath = path.resolve(
        __dirname,
        '..',
        'browser',
        'driver.js',
        ...
        load(id) {
            ...
            if (id === resolvedVirtualModuleId) {
                return /*js*/ `
                    ...
                    export const automationProtocolPath = ${JSON.stringify(
                        automationProtocolPath,
                    )}
                `
            }
        },
            ...
            if (id.includes('.vite/deps/expect.js')) {
                return {
                    code: code
                        ...
                        .replace('process.stdout.isTTY', 'false'),
                }
            }
            return { code }
        },
            ...
            return () => {
                server.middlewares.use('/', async (req, res, next) => {
                    ...
                    if (
                        !urlParsed.pathname ||
                        !urlParsed.path ||
                        !urlParsed.pathname.endsWith('test.html')
                    ) {
                        return next()
                    }
                    ...
                    if (!cid || !SESSIONS.has(cid)) {
                        log.error(
                            `No environment found for ${
                                cid || 'non determined environment'
                            }`,
                        )
                        return next()
                    }
                    ...
                    if (!spec) {
                        log.error(
                            'No spec file was defined to run for this environment',
                        )
                        return next()
                    }
