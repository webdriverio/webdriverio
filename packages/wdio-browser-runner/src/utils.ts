...
import { BROWSER_POOL, EVENTS, PRESET_DEPENDENCIES } from './constants.js'
...
export async function getTemplate(
    ...
    const listeners = Object.entries(EVENTS)
        .map(
            ([
                mochaEvent,
                wdioEvent,
    ...
    if (options.preset === 'vue') {
        try {
            ...
            const vueScript = (
                await fs.readFile(
                    path.join(vueDir, 'dist', 'vue.global.prod.js'),
                    'utf-8',
                )
            ).toString()
            vueDeps += /*html*/ `
            ...
            const vueCompilerDir = path.dirname(
                require.resolve('@vue/compiler-dom'),
            )
            const vueCompilerScript = (
                await fs.readFile(
                    path.join(
                        vueCompilerDir,
                        'dist',
...
export async function getViteConfig(
    ...
    if (options.preset && options.viteConfig) {
        throw new Error(
            'Invalid runner configuration: "preset" and "viteConfig" options are defined but only one of each can be used at the same time',
        )
    }
    ...
    let config: Partial<InlineConfig> = {
        ...
        logLevel: 'silent',
        plugins: [testrunner(options), topLevelAwait()],
        optimizeDeps: {
    ...
    if (options.viteConfig) {
        ...
        const [pkg, importProp, opts] =
            PRESET_DEPENDENCIES[options.preset] || []
        const plugin = (await userfriendlyImport(options.preset, pkg))[
            importProp || 'default'
        ]
        if (plugin) {
...
function handleBrowserCommand(ws: WebSocket) {
    return async (data: Buffer) => {
        try {
            ...
            const browser = (await BROWSER_POOL.get(payload.cid)) as
                | Browser<'async'>
                | undefined
            if (!browser) {
                return ws.send(
            ...
            const result = await (
                browser[payload.commandName as keyof typeof browser] as Function
            )(...payload.args)
            ...
            return ws.send(
                JSON.stringify({
                    error: `Failed handling command message: ${err.message}`,
                }),
            )
        }
    }
