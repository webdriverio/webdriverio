...
class _AccessibilityHandler {
    ...
    constructor (
        ...
        const caps = this._browser.capabilities as Capabilities.Capabilities
    ...
    async before () {
        ...
        (this._browser as BrowserAsync).getAccessibilityResultsSummary = async () => {
    ...
    */
    ...
    private async checkIfPageOpened(browser: Browser<'async'> | MultiRemoteBrowser<'async'>, testIdentifier: string, shouldScanTest?: boolean) {
        let pageOpen = false
        ...
        try {
            const currentURL = await (browser as Browser<'async'>).getUrl()
