...
export default class BrowserstackService implements Services.ServiceInstance {
    ...
    _updateCaps(
        fn: (
            caps: Capabilities.Capabilities | Capabilities.DesiredCapabilities,
        ) => void,
    ) {
        const multiRemoteCap = this
            ._caps as Capabilities.MultiRemoteCapabilities

        if (multiRemoteCap.capabilities) {
            return Object.entries(multiRemoteCap).forEach(([, caps]) =>
                fn(caps.capabilities as Capabilities.Capabilities),
            )
        }
    ...
    afterTest(
        ...
        // Jasmine
        if (test.fullName) {
            const testSuiteName = test.fullName.slice(
                0,
            ...
            if (this._fullTitle === 'Jasmine__TopLevel__Suite') {
                ...
                this._fullTitle = getParentSuiteName(
                    this._fullTitle,
                    testSuiteName,
                )
            }
    ...
    after(result: number) {
        ...
        if (
            this._options.preferScenarioName &&
            this._scenariosThatRan.length === 1
        ) {
    ...
    afterScenario(world: Frameworks.World) {
        ...
        if (status !== 'skipped') {
            this._scenariosThatRan.push(
                world.pickle.name || 'unknown pickle name',
            )
        }
    ...
    async onReload(oldSessionId: string, newSessionId: string) {
        ...
        if (!this._browser.isMultiremote) {
            ...
            const browserName = (
                this._browser as MultiRemoteBrowser<'async'>
            ).instances.filter(
                (browserName: string) =>
                    this._browser &&
    ...
    _isAppAutomate(): boolean {
        ...
        return (
            ...
            !!browserDesiredCapabilities.app ||
            !!desiredCapabilities.app
        )
    }

    _updateJob(requestBody: any) {
        return this._multiRemoteAction(
            (sessionId: string, browserName: string) => {
                log.info(
                    browserName
            ...
            _browser.instances
                .filter((browserName: string) => {
                    const cap = getBrowserCapabilities(
                        _browser,
                        this._caps as Capabilities.MultiRemoteCapabilities,
                        browserName,
                    )
    ...
    _update(sessionId: string, requestBody: any) {
        ...
        log.debug(
            `Updating Browserstack session at ${sessionUrl} with request body: `,
            requestBody,
        )
    ...
    async _printSessionURL() {
        ...
        await this._multiRemoteAction(async (sessionId, browserName) => {
            ...
            log.info(
                `${browserString} session: ${response.body.automation_session.browser_url}`,
            )
        })
    }
}
