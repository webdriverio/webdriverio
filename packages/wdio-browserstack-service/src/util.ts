...
export function getBrowserDescription(cap: Capabilities.DesiredCapabilities) {
     ...
     */
    return BROWSER_DESCRIPTION.map(
        (k: keyof Capabilities.DesiredCapabilities) => cap[k],
    )
        .filter(Boolean)
        .join(' ')
}
...
export function getBrowserCapabilities(
    ...
    const globalCap =
        browserName && browser[browserName]
            ...
            : {}
    const cap =
        browserName && multiCaps[browserName]
            ? multiCaps[browserName].capabilities
            : {}
...
export function getParentSuiteName(
    fullTitle: string,
    ...
    const shortestLength = Math.min(
        fullTitleWords.length,
        testSuiteTitleWords.length,
    )
    let c = 0
    let parentSuiteName = ''
