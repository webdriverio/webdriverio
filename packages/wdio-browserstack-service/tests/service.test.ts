...
describe('onReload()', () => {
    ...
    it('should reset failures', async () => {
        ...
        service['_failReasons'] = [
            'Custom Error: Button should be enabled',
            'Expected something',
        ]
        await service.onReload('1', '2')
        ...
        vi.mocked(got).mockResolvedValue({
            body: {
                automation_session: {
                    ...
                    browser_url:
                        'https://app-automate.browserstack.com/builds/1/sessions/2',
                },
            },
        })
...
describe('before', () => {
    it('should set auth to default values if not provided', async () => {
        let service = new BrowserstackService({} as any, [{}] as any, {
            capabilities: {},
        })

        await service.beforeSession({} as any as any)
        await service.before(
            service['_config'] as any,
            [],
    ...
    it('should initialize correctly', () => {
        ...
        expect(service['_failReasons']).toEqual([])
        expect(service['_sessionBaseUrl']).toEqual(
...
describe('afterTest', () => {
    it('should increment failure reasons on fails', () => {
        ...
        service.afterTest(
            ...
            undefined as never,
            {
                error: { message: 'not so cool reason' },
                result: 1,
                duration: 7,
                passed: false,
            } as any,
    ...
    describe('Jasmine only', () => {
        it('should set suite name of first test as title', () => {
            ...
            service.afterTest(
                { fullName: 'foo bar baz', description: 'baz' } as any,
                undefined as never,
                {} as any,
            )
            expect(service['_fullTitle']).toBe('foo bar')
        ...
        it('should set parent suite name as title', () => {
            ...
            service.afterTest(
                { fullName: 'foo xyz', description: 'xyz' } as any,
                undefined as never,
                {} as any,
            )
    ...
    it('should increment failure reasons on non-passing statuses (strict mode off)', () => {
        ...
        service.afterScenario({
            ...
            result: {
                ...
                willBeRetried: false,
                status: 'FAILED',
                message: 'I am Error, most likely',
            },
        })
        ...
        service.afterScenario({
            ...
            result: {
                ...
                willBeRetried: false,
                status: 'AMBIGUOUS',
                message: 'Step XYZ2 is ambiguous',
            },
        })
        expect(service['_failReasons']).toEqual([
            'I am Error, most likely',
    ...
    describe('Cucumber only', function () {
        it('should call _update with status "failed" if strict mode is "on" and all tests are pending', async () => {
            ...
            await service.afterScenario({
                pickle: { name: 'Can do something but pending 1' },
                result: { status: 'PENDING' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something but pending 2' },
                result: { status: 'PENDING' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something but pending 3' },
                result: { status: 'PENDING' } as any,
            })
        ...
        it('should call _update with status is "failed" when strict mode is "on" and only passed and pending tests ran', async () => {
            ...
            await service.afterScenario({
                pickle: { name: 'Can do something 1' },
                result: { status: 'PASSED' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something but pending' },
                result: { status: 'PENDING' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something 2' },
                result: { status: 'PASSED' } as any,
            })
        ...
        it('should call _update with status "passed" when all tests are skipped', async () => {
            ...
            await service.afterScenario({
                pickle: { name: 'Can do something skipped 1' },
                result: { status: 'SKIPPED' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something skipped 2' },
                result: { status: 'SKIPPED' } as any,
            })
            await service.afterScenario({
                pickle: { name: 'Can do something skipped 3' },
                result: { status: 'SKIPPED' } as any,
            })
        ...
        it('should call _update with status "failed" when strict mode is "on" and only failed and pending tests ran', async () => {
            ...
            await service.afterScenario({
                pickle: { name: 'Can do something failed 1' },
                result: {
                    message: 'I am error, hear me roar',
                    status: 'FAILED',
                } as any,
            })
            await service.afterScenario({
            ...
            await service.afterScenario({
                pickle: { name: 'Can do something but passed 3' },
                result: { status: 'SKIPPED' } as any,
            })
            ...
            describe('enabled', () => {
                ;[
                    ...
                    it(`should call _update /w status failed and name of Scenario when single "${status}" Scenario ran`, async () => {
                        service = new BrowserstackService(
                            { preferScenarioName: true } as any,
                            [] as any,
                            {
                                user: 'foo',
                                key: 'bar',
            ...
            describe('disabled', () => {
                ;['FAILED', 'AMBIGUOUS', 'UNDEFINED', 'UNKNOWN'].map((status) =>
                    it(`should call _update /w status failed and name of Feature when single "${status}" Scenario ran`, async () => {
                        service = new BrowserstackService(
                            { preferScenarioName: false } as any,
                            [] as any,
                            {
                                user: 'foo',
                                key: 'bar',
