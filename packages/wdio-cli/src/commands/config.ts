...
const runConfig = async function (
    ...
    const runnerPackage = convertPackageHashToObject(
        answers.runner || SUPPORTED_PACKAGES.runner[0].value,
    )
    const servicePackages = answers.services.map((service) =>
        convertPackageHashToObject(service),
    )
    const pluginPackages = answers.plugins.map((plugin) =>
        convertPackageHashToObject(plugin),
    )
    const reporterPackages = answers.reporters.map((reporter) =>
        convertPackageHashToObject(reporter),
    )
    ...
    if (parsedAnswers.npmInstall) {
        ...
        if (result.status !== 0) {
            const customError =
                'An unknown error happened! Please retry ' +
                `installing dependencies via "${
                    useYarn ? 'yarn add --dev' : 'npm i --save-dev'
                } ` +
                `${packagesToInstall.join(' ')}"\n\nError: ${
        ...
        console.log(
            util.format(DEPENDENCIES_INSTALLATION_MESSAGE, installationCommand),
        )
    }
 ...
 */
export async function missingConfigurationPrompt(
    command: string,
    message: string,
    useYarn = false,
    runConfigCmd = runConfig,
) {
    const configMessage =
        command === 'run'
