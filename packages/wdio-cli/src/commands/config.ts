...
export const parseAnswers = async function (yes: boolean): Promise<ParsedAnswers> {
    ...
    const tsConfigFilePath = !hasRootTSConfig
        ...
        : answers.specs
            /**
             * if we have autogenerated spec files, put the tsconfig two levels above the spec file dir
             */
            ? path.resolve(
                path.dirname(path.dirname(answers.specs.split(path.sep).filter((s) => !s.includes('*')).join(path.sep))),
                'tsconfig.json'
            )
            /**
...
export async function missingConfigurationPrompt(command: string, message: string, useYarn = false, runConfigCmd = runConfigCommand) {
    const configMessage = command === 'run'
        ? `Error: Could not execute "run" due to missing configuration, file "${path.parse(message).name}[.js/.ts]" not found! Would you like to create one?`
