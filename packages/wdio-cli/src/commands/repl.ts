...
export const builder = (yargs: Argv) => {
    return yargs
        .options(
            pickBy(
                { ...cmdArgs, ...runCmdArgs },
                (_, key) => !IGNORED_ARGS.includes(key),
            ),
        )
        .example('$0 repl firefox --path /', 'Run repl locally')
        .example(
            '$0 repl chrome -u <SAUCE_USERNAME> -k <SAUCE_ACCESS_KEY>',
            'Run repl in Sauce Labs cloud',
        )
        .example(
            '$0 repl android',
            'Run repl browser on launched Android device',
        )
        .example(
            '$0 repl "./path/to/your_app.app"',
            'Run repl native app on iOS simulator',
        )
        .example(
            '$0 repl ios -v 11.3 -d "iPhone 7" -u 123432abc',
            'Run repl browser on iOS device with capabilities',
        )
        .example(
            '$0 repl "./path/to/wdio.config.js" 0 -p 9515',
            'Run repl using the first capability from the capabilty array in wdio.config.js',
        )
        .example(
            '$0 repl "./path/to/wdio.config.js" "myChromeBrowser" -p 9515',
            'Run repl using a named multiremote capabilities in wdio.config.js',
        )
