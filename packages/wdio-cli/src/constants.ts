...
export const QUESTIONNAIRE = [{
    ...
    when: /* istanbul ignore next */ (answers: Questionnair) => String(answers.backend).indexOf('different service') > -1
}, {
    type: 'input',
    name: 'port',
    ...
    default: 'LT_USERNAME',
    when: /* istanbul ignore next */ (answers: Questionnair) => (
        String(answers.backend).indexOf('LambdaTest') > -1 &&
    ...
    default: 'localhost',
    when: /* istanbul ignore next */ (answers: Questionnair) => String(answers.backend).indexOf('own Selenium cloud') > -1
}, {
    type: 'input',
    name: 'port',
