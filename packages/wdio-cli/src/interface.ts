...
export default class WDIOCLInterface extends EventEmitter {
    ...
    private _jobs: Map<string, Capabilities.Job> = new Map()
    ...
    onSpecRetry (rid: string, job: Capabilities.Job, retries: number) {
    ...
    onSpecPass (rid: string, job: Capabilities.Job, retries: number) {
        this.onJobComplete(rid, job, retries, chalk.bold.green('PASSED'))
    }

    onSpecFailure (rid: string, job: Capabilities.Job, retries: number) {
        this.onJobComplete(rid, job, retries, chalk.bold.red('FAILED'))
    }

    onSpecSkip (rid: string, job: Capabilities.Job) {
        this.onJobComplete(rid, job, 0, 'SKIPPED', log.info)
    }

    onJobComplete(cid: string, job?: Capabilities.Job, retries = 0, message = '', _logger: Function = this.log) {
     ...
     */
    addJob ({ cid, caps, specs, hasTests }: Capabilities.Job & { cid: string }) {
