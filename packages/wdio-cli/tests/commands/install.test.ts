...
You can create one by running 'wdio config'`,
    ...
    it('should fail if config could not be updated', async () => {
        ...
        expect(err.message).toBe(
            'Couldn\'t find "service" property in wdio.conf.js',
        )
    })
    ...
    it('should exit if there is an error while installing', async () => {
        ...
        vi.mocked(yarnInstall).mockImplementation(
            () => ({ status: 1, stderr: 'test error' } as any),
        )
