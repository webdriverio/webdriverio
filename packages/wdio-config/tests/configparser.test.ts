...
import {
    FileNamed,
    realReadFilePair,
    realRequiredFilePair,
} from './lib/FileNamed.js'
import MockFileContentBuilder, {
    MockFileContent,
} from './lib/MockFileContentBuilder.js'
import {
    FilePathsAndContents,
    MockSystemFilePath,
    MockSystemFolderPath,
} from './lib/MockPathService.js'
...
const FIXTURES_DEFAULT_CONF = path.resolve(
    FIXTURES_PATH,
    'wdio.default.conf.ts',
)
const FIXTURES_CUCUMBER_FEATURE_A_LINE_2 = path.resolve(
    FIXTURES_PATH,
    'test-a.feature:2',
)
const FIXTURES_CUCUMBER_FEATURE_A_LINE_2_AND_12 = path.resolve(
    FIXTURES_PATH,
    'test-a.feature:2:12',
)
const FIXTURES_CUCUMBER_FEATURE_B_LINE_7 = path.resolve(
    FIXTURES_PATH,
    'test-b.feature:7',
)
...
const TestWdioConfig_AllInMemory = (): MockFileContent => ({
    config: {
        ...
        exclude: [path.join(__dirname, '/tests//validateConfig.test.ts')],
        capabilities: [
            {
                browserName: 'chrome',
            },
...
async function MockedFileSystem_LoadingAsMuchAsCanFromFileSystem(): Promise<FilePathsAndContents> {
    return [
        realReadFilePair(
            path.resolve(FIXTURES_PATH, '../validateConfig.test.ts'),
        ),
        realReadFilePair(
            path.resolve(FIXTURES_PATH, '../configparser.test.ts'),
        ),
        realReadFilePair(path.resolve(FIXTURES_PATH, '../utils.test.ts')),
        realReadFilePair(
            path.resolve(FIXTURES_PATH, '../RequireLibrary.test.ts'),
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test.cjs')).withContents(
            'test file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test.es6')).withContents(
            'test file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test.java')).withContents(
            'test file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test.mjs')).withContents(
            'test file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test-a.feature')).withContents(
            'feature file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'test-b.feature')).withContents(
            'feature file contents',
        ),
        FileNamed(path.resolve(FIXTURES_PATH, 'typescript.ts')).withContents(
            'test file contents',
        ),
        await realRequiredFilePair(
            path.resolve(FIXTURES_PATH, 'wdio.array.conf.ts'),
        ),
        await realRequiredFilePair(
            path.resolve(FIXTURES_PATH, 'wdio.conf.multiremote.rdc.ts'),
        ),
        await realRequiredFilePair(
            path.resolve(FIXTURES_PATH, 'wdio.conf.rdc.ts'),
        ),
        await realRequiredFilePair(path.resolve(FIXTURES_PATH, 'wdio.conf.ts')),
        await realRequiredFilePair(
            path.resolve(FIXTURES_PATH, 'wdio.local.conf.ts'),
        ),
 ...
 */
// eslint-disable-next-line camelcase
function MockedFileSystem_OnlyLoadingConfig(
    baseDir: MockSystemFolderPath,
    configFilepath: MockSystemFilePath,
): FilePathsAndContents {
    return [
        FileNamed(configFilepath).withContents(TestWdioConfig_AllInMemory()),
        FileNamed(path.join(baseDir, '../validateConfig.test.ts')).withContents(
            'test contents',
        ),
        FileNamed(path.join(baseDir, '../configparser.test.ts')).withContents(
            'test contents',
        ),
        FileNamed(path.join(baseDir, '../utils.test.ts')).withContents(
            'test contents',
        ),
        FileNamed(path.join(baseDir, '../RequireLibrary.test.ts')).withContents(
            'test contents',
        ),
        ...
        FileNamed(path.join(baseDir, 'test.java')).withContents(
            'test contents',
        ),
        FileNamed(path.join(baseDir, 'test.mjs')).withContents('test contents'),
        FileNamed(path.join(baseDir, 'test-a.feature')).withContents(
            'feature file contents',
        ),
        FileNamed(path.join(baseDir, 'test-b.feature')).withContents(
            'feature file contents',
        ),
        FileNamed(path.join(baseDir, 'typescript.ts')).withContents(
            'test contents',
        ),
        FileNamed(
            path.join(baseDir, 'wdio.conf.multiremote.rdc.ts'),
        ).withContents('config contents'),
        FileNamed(path.join(baseDir, 'wdio.conf.rdc.ts')).withContents(
            'config contents',
        ),
        FileNamed(path.join(baseDir, 'wdio.conf.ts')).withContents(
            'config contents',
        ),
        FileNamed(path.join(baseDir, 'wdio.local.conf.ts')).withContents(
            'config contents',
        ),
        FileNamed(path.join(baseDir, '../app/src/index.ts')).withContents(
            'source contents',
        ),
    ]
}
...
function ConfigParserForTest(config = FIXTURES_CONF) {
    return ConfigParserBuilder.withBaseDir(FIXTURES_PATH, config)
        .withFiles(MockedFileSystem_OnlyLoadingConfig(FIXTURES_PATH, config))
        .build()
}
...
describe('ConfigParser', () => {
    it('should throw if getFilePaths is not a string', () => {
        expect(() =>
            ConfigParser.getFilePaths(123 as any, '/foo/bar'),
        ).toThrow()
    })
    ...
    describe('addConfigFile', () => {
        it('should throw if config file is not a string', async () => {
            ...
            await expect(() =>
                configParser['addConfigFile'](123 as any),
            ).rejects.toThrow()
        })
        ...
        it('should throw if config file does not exist (absolute path)', async () => {
            ...
            expect(() =>
                configParser['addConfigFile'](
                    path.resolve(__dirname, 'foobar.conf.ts'),
                ),
            ).rejects.toThrow()
        })
        ...
        it('should throw if config file does not exist (relative path)', async () => {
            const configParser = await ConfigParserForTest()
            expect(() =>
                configParser['addConfigFile']('foobar.conf.ts'),
            ).rejects.toThrow()
        })
        ...
        it('should throw if config file is not a config file (absolute path)', async () => {
            ...
            expect(() =>
                configParser['addConfigFile'](
                    path.resolve(FIXTURES_PATH, 'test-a.feature'),
                ),
            ).rejects.toThrow()
        })
        ...
        it('should throw if config file is not a config file (relative path)', async () => {
            const configParser = await ConfigParserForTest()
            expect(() =>
                configParser['addConfigFile']('test-a.feature'),
            ).rejects.toThrow()
        })
        ...
        describe('TypeScript integration', () => {
            ...
            it('when ts-node exists should initiate TypeScript compiler with defaults', async function () {
                ...
                const configParser = ConfigParserBuilder.withBaseDir(
                    ...
                    .withFiles([
                        ...
                        FileNamed(
                            path.join(FIXTURES_PATH, '/here/cool.conf'),
                        ).withContents(configFileContents),
                    ])
                    .build()
            ...
            it('when ts-node exists should initiate TypeScript compiler with defaults if autoCompiled before config is read', async function () {
                ...
                const configParser = ConfigParserBuilder.withBaseDir(
                    ...
                    .withFiles([
                        ...MockedFileSystem_OnlyLoadingConfig(
                            path.join(__dirname, '/tests/'),
                            '/path/to/config',
                        ),
                        FileNamed(
                            path.join(__dirname, '/tests/tests/cool.conf'),
                        ).withContents(JSON.stringify(configFileContents)),
                    ])
                    .withTsNodeModule(tsNodeRegister)
                    .build()
                await configParser.initialize()
            ...
            it('should just continue without initiation if ts-node does not exist', async () => {
                ...
                expect(
                    (log.debug as MockedFunction<any>).mock.calls[0][0],
        ...
        describe('Babel integration', () => {
            ...
            it('when @babel/register package exists should initiate with @babel/register compiler', async () => {
                ...
                const configParser = ConfigParserBuilder.withBaseDir(
                    ...
                    .withFiles([
                        FileNamed(FIXTURES_CONF_RDC).withContents(
                            configContents,
                        ),
                    ])
            ...
            it('when @babel/register package exists should merge config, preferring config, if present', async function () {
                ...
                const configParser = ConfigParserBuilder.withBaseDir(
                    ...
                    .withFiles([
                        ...
                        FileNamed(
                            path.join(__dirname, '/tests/configparser.test.ts'),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(__dirname, '/tests/utils.test.ts'),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/RequireLibrary.test.ts',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test.cjs',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test.es6',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test.java',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test.mjs',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test-a.feature',
                            ),
                        ).withContents('feature file contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/test-b.feature',
                            ),
                        ).withContents('feature file contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/typescript.ts',
                            ),
                        ).withContents('test contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/wdio.conf.multiremote.rdc.ts',
                            ),
                        ).withContents('config contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/wdio.conf.rdc.ts',
                            ),
                        ).withContents('config contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/wdio.conf.ts',
                            ),
                        ).withContents('config contents'),
                        FileNamed(
                            path.join(
                                __dirname,
                                '/tests/__fixtures__/wdio.local.conf.ts',
                            ),
                        ).withContents('config contents'),
                        FileNamed(
                            path.join(__dirname, '/app/src/index.ts'),
                        ).withContents('source contents'),
                    ])
                    .withBabelModule(babelRegister)
                    .build()
                await configParser.initialize()
            ...
            it('should just continue without initiation when autoCompile:false', async () => {
                process.env.THROW_BABEL_REGISTER = '1'
                const configParserBuilder = ConfigParserBuilder.withBaseDir(
                    FIXTURES_PATH,
                    FIXTURES_CONF_RDC,
                    {
                        autoCompileOpts: {
                ...
                const { requireMock } = configParserBuilder
                    .getMocks()
                    .modules.getMocks()
            ...
            it('when both ts-node and @babel/register exist should prefer ts-node', async () => {
                ...
                expect(log.debug).toHaveBeenCalledWith(
                    expect.stringContaining('auto-compiling TypeScript files'),
                )
            })
        })
    })
    ...
    describe('merge', () => {
        ...
        it('should allow specifying a spec file which is Cucumber feature file with line number', async () => {
            ...
            await configParser.initialize({
                spec: [FIXTURES_CUCUMBER_FEATURE_A_LINE_2],
            })
            ...
            if (isWindows) {
                featureFileWithoutLine =
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2.split(':')[0] +
                    ':' +
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2.split(':')[1]
            } else {
                featureFileWithoutLine =
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2.split(':')[0]
            }
        ...
        it('should allow specifying a spec file which is Cucumber feature file with line numbers', async () => {
            ...
            configParser['merge']({
                spec: [FIXTURES_CUCUMBER_FEATURE_A_LINE_2_AND_12],
            })
            ...
            if (isWindows) {
                featureFileWithoutLine =
                    ...
                    ':' +
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2_AND_12.split(':')[1]
            } else {
                featureFileWithoutLine =
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2_AND_12.split(':')[0]
            }
        ...
        it('should allow specifying a spec file which is Cucumber feature files with line number', async () => {
            ...
            if (isWindows) {
                featureFileA =
                    ...
                    FIXTURES_CUCUMBER_FEATURE_A_LINE_2.split(':')[1]
                featureFileB =
                    FIXTURES_CUCUMBER_FEATURE_B_LINE_7.split(':')[0] +
                    ':' +
                    FIXTURES_CUCUMBER_FEATURE_B_LINE_7.split(':')[1]
            } else {
        ...
        it('should allow to specify partial matching spec file', async () => {
            ...
            expect(specs).toContain(
                path.join(__dirname, 'RequireLibrary.test.ts'),
            )
        })
        ...
        it('should throw if specified spec file does not exist', async () => {
            const configParser = await ConfigParserForTest(FIXTURES_CONF)
            const hasError = await configParser
                .initialize({ spec: [path.resolve(__dirname, 'foobar.ts')] })
                .then(
                    () => false,
                    () => true,
                )
        ...
        it('should allow to specify multiple suites', async () => {
            ...
            expect(specs).not.toContain(
                path.join(__dirname, 'validateConfig.test.ts'),
            )
            expect(specs).toContain(
                path.join(__dirname, 'RequireLibrary.test.ts'),
            )
        ...
        it('should allow specifying a glob pattern for exclude', async () => {
            ...
            configParser['merge']({
                exclude: [
                    path.join(__dirname, '**', '*conf*').replace(/\\/g, '/'),
                ],
            })
        ...
        it('should overwrite config and capabilities exclude if piped into cli command', async () => {
            ...
            const specs = configParser.getSpecs(
                [FIXTURES_CONF, FIXTURES_CONF_RDC],
                [FIXTURES_CONF_RDC],
            )
        ...
        it('should overwrite config and capabilities exclude if piped into cli command with suite', async () => {
            ...
            const requireLibPath = path.join(
                __dirname,
                'RequireLibrary.test.ts',
            )
            const configParserPath = path.join(
                __dirname,
            ...
            const specs = configParser.getSpecs(
                [configParserPath, requireLibPath],
                [configParserPath],
            )
        ...
        it('should overwrite capabilities', async () => {
            const configParser = await ConfigParserForTest(FIXTURES_CONF)
            await configParser.initialize()
            expect(configParser.getCapabilities()).toMatchObject([
                { browserName: 'chrome' },
            ])
            configParser['merge']({
    ...
    describe('getCapabilities', () => {
        it('allows to grab certain capabilities', async () => {
            ...
            expect(configParser.getCapabilities()).toEqual(
                configParser['_capabilities'],
            )
            expect(configParser.getCapabilities(1)).toEqual({
                browserName: 'bar',
            })
        })
    })
    ...
    describe('getSpecs', () => {
        ...
        it('should exclude/include capability excludes in suites', async () => {
            ...
            const getSpecs = () =>
                configParser.getSpecs([INDEX_PATH], [requireLibPath])
        ...
        it('should include typescript files', async () => {
            ...
            expect(specs).toContain(
                path.resolve(FIXTURES_PATH, 'typescript.ts'),
            )
        })
        ...
        it('should include spec when specifying a suite unless excluded', async () => {
            const configParser = ConfigParserBuilder.withBaseDir(
                ...
                .withFiles([
                    ...
                    FileNamed(
                        path.resolve(__dirname, 'zelda.test.ts'),
                    ).withContents(''),
                ])
                .build()
        ...
        it("should throw if suite doesn't exist or doesn't contain any files", async () => {
            const configParser = ConfigParserBuilder.withBaseDir(
                ...
                .withFiles([
                    FileNamed(
                        path.join(process.cwd(), '/workdir/conf-under-test'),
                    ).withContents({
                        config: {
                            user: 'foobar',
                            key: '50fa142c-3121-4gb0-9p07-8q326vvbq7b0',
                            ...
                            exclude: [
                                path.join(
                                    __dirname,
                                    '/tests/and-this-test-two.test.ts',
                                ),
                            ],
                            capabilities: [
                                {
                    ...
                    FileNamed(
                        path.join(
                            __dirname,
                            '/tests/only-this-test-one.test.ts',
                        ),
                    ).withContents('what1'),
                ])
                .build()
            ...
            expect(() => configParser.getSpecs()).toThrowError(
                'The suite(s) "something" you specified don\'t exist in your config file or doesn\'t contain any files!',
            )
            configParser['merge']({
                suite: ['something-else'],
                spec: [
                    path.join(__dirname, '/tests/only-this-test-one.test.ts'),
                ],
            })
            // eslint-disable-next-line no-useless-escape
            expect(() => configParser.getSpecs()).toThrowError(
                'The suite(s) "something", "something-else" you specified don\'t exist in your config file or doesn\'t contain any files!',
            )
        })
    })
    ...
    describe('getFilePaths', () => {
        it('should include files in arrays to be run in a single worker', async () => {
            ...
            const filePaths = ConfigParser.getFilePaths(
                configParser['_config'].specs!,
                FIXTURES_CONF_ARRAY,
                configParser['_pathService'],
            )
