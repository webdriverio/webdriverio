...
const EventDataCollector =
    require('@cucumber/cucumber/lib/formatter/helpers/event_data_collector').default
...
class CucumberAdapter {
    ...
    constructor(
        ...
        this._eventDataCollector = new EventDataCollector(
        ...
        const featurePathsToRun =
            this._cucumberFeaturesWithLineNumbers.length > 0
                ? this._cucumberFeaturesWithLineNumbers
                : this._specs
        ...
        const reporterOptions = {
            ...
            ignoreUndefinedDefinitions: Boolean(
                this._cucumberOpts.ignoreUndefinedDefinitions,
            ),
            failAmbiguousDefinitions: Boolean(
                this._cucumberOpts.failAmbiguousDefinitions,
            ),
            tagsInTitle: Boolean(this._cucumberOpts.tagsInTitle),
            scenarioLevelReporter: Boolean(
                this._cucumberOpts.scenarioLevelReporter,
            ),
        }
        this._cucumberReporter = new CucumberReporter(
            this._eventBroadcaster,
            this._pickleFilter,
            reporterOptions,
            this._cid,
    ...
    async init() {
        try {
            ...
            this._hasTests =
                this._cucumberReporter.eventListener.getPickleIds(
                    this._capabilities,
                ).length > 0
        } catch (runtimeError) {
    ...
    async run() {
        ...
        try {
            ...
            const supportCodeLibrary =
                Cucumber.supportCodeLibraryBuilder.finalize()
             ...
             */
            this.getHookParams =
                this._cucumberReporter.eventListener.getHookParams.bind(
                    this._cucumberReporter.eventListener,
                )
            ...
            const runtime = new Cucumber.Runtime({
                ...
                supportCodeLibrary,
                eventDataCollector: this._eventDataCollector,
                pickleIds: this._cucumberReporter!.eventListener.getPickleIds(
                    this._capabilities,
                ),
            })
    ...
    requiredFiles() {
        return this._cucumberOpts.require.reduce(
            (files: string[], requiredFile: string) =>
                files.concat(
                    isGlob(requiredFile)
    ...
    async loadSpecFiles() {
        ...
        await Promise.all(
            this.requiredFiles().map((codePath) => {
    ...
    addWdioHooks(config: Options.Testrunner) {
        ...
        Cucumber.AfterStep(async function wdioHookAfterStep(
            world: ITestCaseHookParameter,
        ) {
            const params = eventListener?.getHookParams()
    ...
    wrapStep(
        ...
        return function (this: Cucumber.World, ...args: any[]) {
            ...
            const retryTest =
                isStep && isFinite(options.retry) ? options.retry : 0
            ...
            return testFnWrapper.call(
                ...
                {
                    beforeFn: beforeFn as Function[],
                    beforeFnArgs: (context) => [hookParams?.step, context],
                },
                {
                    afterFn: afterFn as Function[],
                    afterFnArgs: (context) => [hookParams?.step, context],
                },
                cid,
                retryTest,
            )
