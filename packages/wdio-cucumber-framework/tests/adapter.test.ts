...
import CucumberAdapter, * as packageExports from '../src/index.js'
...
vi.mock('../src/reporter', () => ({
    default: class CucumberReporter {
        eventListener = {
            ...
            getHookParams: vi.fn().mockReturnValue({
                uri: 'uri',
                feature: 'feature',
                scenario: 'scenario',
                step: 'step',
...
vi.mock('@cucumber/gherkin-streams', () => ({
    GherkinStreams: {
        fromPaths: vi.fn().mockReturnValue('GherkinStreams.fromPaths'),
    },
}))
...
describe('CucumberAdapter', () => {
    ...
    it('exports Cucumber exports', () => {
        expect(Object.keys(packageExports)).toContain('Given')
        expect(Object.keys(packageExports)).toContain('When')
        expect(Object.keys(packageExports)).toContain('Then')
        expect(Object.keys(packageExports)).toContain('Before')
        expect(Object.keys(packageExports)).toContain('After')
        expect(Object.keys(packageExports)).toContain('World')
        expect(Object.keys(packageExports)).toContain('DataTable')
    })
    ...
    it('should trigger after hook if initiation fails', async () => {
        vi.mocked(Cucumber.parseGherkinMessageStream).mockRejectedValueOnce(
            new Error('boom'),
        )

        const err = await CucumberAdapter.init!(
            '0-0',
            {
    ...
    it.skip('loadSpecFiles', async () => {
        ...
        adapter.requiredFiles = vi
            .fn()
            .mockReturnValue([__dirname + '/__mocks__/moduleC.ts'])
        ...
        expect(mockery.registerMock).toBeCalledWith(
            '@cucumber/cucumber',
            expect.any(Object),
        )
    ...
    it('wrapSteps', async () => {
        ...
        vi.mocked(Cucumber.setDefinitionFunctionWrapper).mock.calls[0][0](
            vi.fn(),
        )
    ...
    it('wrapSteps does not wrap wdio hooks', async () => {
        ...
        expect(
            vi
                .mocked(Cucumber.setDefinitionFunctionWrapper)
                .mock.calls[0][0](wdioHookFn)(),
        ).toBe('foobar')
    })
    ...
    it('wrapStep', async () => {
        ...
        const wrappedStep = adapter.wrapStep(
            'code',
            true,
            {},
            '0-2',
