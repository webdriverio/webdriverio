 ...
 * browser session without exiting; `exit`/`quit` shuts down cleanly
 * (closing the MCP server); Ctrl-C cancels a running turn or exits when idle.
 *
 ...
 */
export async function runRepl(agent: DeepAgent, onClose: () => Promise<void>, closeSession?: () => Promise<void>, onFirstSubmit?: () => void): Promise<void> {
