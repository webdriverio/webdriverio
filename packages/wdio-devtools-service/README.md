...
> A WebdriverIO service that integrates Google Lighthouse commands to use it for automate tests.

<!-- @ToDo(Christian): come up with a service description -->
tbd...

## Installation

The easiest way is to keep `@wdio/lighthouse-service` as a `devDependency` in your `package.json`, via:

```sh
npm install @wdio/devtools-lighthouse --save-dev
```
...
To use the service you just need to add the service to your service list in your `wdio.conf.js`, like:
...
The `@wdio/lighthouse-service` offers you running Google Lighthouse reports with WebdriverIO.
...
With the `checkPWA` command, you can validate if your web application is compliant with the latest web standards when it comes to progressive web apps. It checks:
...
Furthermore, it exposes the utility methods the service is using for its performance analysis.
...
Returns the trace logs that were captured within the tracing period. You can use this command to store the trace logs on the file system to analyze the trace via the Chrome DevTools interface.
