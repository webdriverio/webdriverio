...
export default class Auditor {
    ...
    updateCommands(
        ...
        customFn?: CustomInstanceCommands<Browser<'async'>>['addCommand'],
    ) {
        const commands = Object.getOwnPropertyNames(
            ...
            (fnName) =>
                fnName !== 'constructor' &&
                fnName !== 'updateCommands' &&
                !fnName.startsWith('_'),
        )
        commands.forEach((fnName) =>
            browser.addCommand(
                fnName,
                customFn || (this[fnName as keyof Auditor] as any).bind(this),
            ),
        )
    }
    ...
    async getMainThreadWorkBreakdown() {
        const result = (await this._audit(
            MainThreadWorkBreakdown,
        )) as MainThreadWorkBreakdownResult
        return result.details.items.map(({ group, duration }) => ({
            group,
            duration,
        }))
    }
     ...
     */
    async getMetrics() {
        const serverResponseTime = (await this._audit(ServerResponseTime, {
            URL: this._url,
        })) as ResponseTimeResult
        const cumulativeLayoutShift = (await this._audit(
    ...
    async getPerformanceScore() {
        const auditResults: AuditResults = {
            ...
            'first-contentful-paint': (await this._audit(
                FirstContentfulPaint,
            )) as MetricsResult,
            'largest-contentful-paint': (await this._audit(
                LargestContentfulPaint,
            )) as MetricsResult,
            'cumulative-layout-shift': (await this._audit(
                CumulativeLayoutShift,
            )) as MetricsResult,
            'total-blocking-time': (await this._audit(
                TotalBlockingTime,
            )) as MetricsResult,
            interactive: (await this._audit(
                InteractiveMetric,
            )) as MetricsResult,
        }
        ...
        if (
            ...
            !auditResults['cumulative-layout-shift'] ||
            !auditResults['first-contentful-paint'] ||
            !auditResults['largest-contentful-paint'] ||
            !auditResults['speed-index'] ||
            !auditResults['total-blocking-time']
        ) {
            log.info(
                "One or multiple required metrics couldn't be found, setting performance score to: null",
            )
            return null
        }

        const scores = defaultConfig.categories.performance.auditRefs
            .filter((auditRef: AuditRef) => auditRef.weight)
            .map((auditRef: AuditRef) => ({
    ...
    async _auditPWA(
        ...
        const audits: RunAuditResult[] = await Promise.all(
            Object.entries(PWA_AUDITS)
                ...
                .map<Promise<RunAuditResult>>(async ([name, Audit]) => [
                    name,
                    await this._audit(Audit, params),
                ]),
        )
        return {
