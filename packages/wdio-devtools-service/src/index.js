...
export default class DevToolsService {
    constructor(serviceOptions, specs, config, browser) {
    ...
    async onReload() {
        ...
        this.browser.puppeteer = null
        return this._setupHandler()
    }

    async before() {
        this.isSupported = this.isSupported || Boolean(this.browser.puppeteer)
        return this._setupHandler()
    }
    ...
    async afterCommand(commandName) {
        ...
        this.traceGatherer.once('tracingComplete', (traceEvents) => {
            ...
            auditor.updateCommands(this.browser)
        })

        this.traceGatherer.once('tracingError', (err) => {
            const auditor = new Auditor()
            auditor.updateCommands(this.browser, /* istanbul ignore next */() => {
    ...
    async _setupHandler() {
        if (!this.isSupported) {
            return setUnsupportedCommand(this.browser)
        }
        ...
        this.commandHandler = new CommandHandler(this.session, this.page, this.browser)
        ...
        this.puppeteer._connection._transport._ws.addEventListener('message', (event) => {
            ...
            this.browser.emit(method, data.params)
        })

        this.browser.addCommand('enablePerformanceAudits', this._enablePerformanceAudits.bind(this))
        this.browser.addCommand('disablePerformanceAudits', this._disablePerformanceAudits.bind(this))
        this.browser.addCommand('emulateDevice', this._emulateDevice.bind(this))
    }
}
