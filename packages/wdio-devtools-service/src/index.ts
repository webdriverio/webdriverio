...
import DevtoolsGatherer, {
    CDPSessionOnMessageObject,
} from './gatherer/devtools.js'
...
export default class DevToolsService implements Services.ServiceInstance {
    ...
    before(
        ...
        this._isSupported =
            this._isSupported || Boolean(this._browser.puppeteer)
    ...
    async beforeCommand(commandName: string, params: any[]) {
        const isCommandNavigation = ['url', 'navigateTo'].some(
            (cmdName) => cmdName === commandName,
        )
        if (
            !this._shouldRunPerformanceAudits ||
            !this._traceGatherer ||
            this._traceGatherer.isTracing ||
            !TRACE_COMMANDS.includes(commandName)
        ) {
            return
        }
         ...
         * set browser profile
         */
        this._setThrottlingProfile(
            this._networkThrottling,
            this._cpuThrottling,
            this._cacheEnabled,
        )
    ...
    async afterCommand(commandName: string) {
        ...
        this._traceGatherer.once('tracingComplete', (traceEvents) => {
            const auditor = new Auditor(
                traceEvents,
                this._devtoolsGatherer?.getLogs(),
                this._formFactor,
            )
        ...
        this._traceGatherer.once('tracingError', (err: Error) => {
            ...
            auditor.updateCommands(
                this._browser as Browser<'async'>,
                /* istanbul ignore next */ () => {
                    throw new Error(
                        `Couldn't capture performance due to: ${err.message}`,
                    )
                },
            )
        })
    ...
    _enablePerformanceAudits({
        ...
        cacheEnabled,
        formFactor,
    }: EnablePerformanceAuditsOptions = DEFAULT_THROTTLE_STATE) {
        if (!NETWORK_STATES[networkThrottling]) {
            throw new Error(
                `Network throttling profile "${networkThrottling}" is unknown, choose between ${Object.keys(
                    NETWORK_STATES,
                ).join(', ')}`,
            )
        }
        ...
        if (typeof cpuThrottling !== 'number') {
            throw new Error(
                `CPU throttling rate needs to be typeof number but was "${typeof cpuThrottling}"`,
            )
        }
     ...
     */
    async _emulateDevice(
        device: string | DeviceDescription,
        inLandscape?: boolean,
    ) {
        if (!this._page) {
        ...
        if (typeof device === 'string') {
            const deviceName = (device +
                (inLandscape ? ' landscape' : '')) as keyof typeof KnownDevices
            ...
            if (!deviceCapabilities) {
                ...
                throw new Error(
                    `Unknown device, available options: ${deviceNames.join(
                        ', ',
                    )}`,
                )
            }
    ...
    async _setThrottlingProfile(
        ...
        await this._session.send('Emulation.setCPUThrottlingRate', {
            rate: cpuThrottling,
        })
        await this._session.send(
            'Network.emulateNetworkConditions',
            NETWORK_STATES[networkThrottling],
        )
    }
    ...
    async _setupHandler() {
         ...
         */
        this._puppeteer = (await (
            this._browser as Browser<'async'>
        ).getPuppeteer()) as any as PuppeteerBrowser
        ...
        this._traceGatherer = new TraceGatherer(
            this._session,
            this._page,
            this._driver,
        )
        ...
        this._session.on(
            'Page.loadEventFired',
            this._traceGatherer.onLoadEventFired.bind(this._traceGatherer),
        )
        this._session.on(
            'Page.frameNavigated',
            this._traceGatherer.onFrameNavigated.bind(this._traceGatherer),
        )

        this._page.on(
            'requestfailed',
            this._traceGatherer.onFrameLoadFail.bind(this._traceGatherer),
        )
         ...
         */
        await Promise.all(
            ['Page', 'Network', 'Runtime'].map((domain) =>
                Promise.all([this._session?.send(`${domain}.enable` as any)]),
            ),
        )
        ...
        if (this._options.coverageReporter?.enable) {
            this._coverageGatherer = new CoverageGatherer(
                this._page,
                this._options.coverageReporter,
            )
            this._browser.addCommand(
                'getCoverageReport',
                this._getCoverageReport.bind(this),
            )
        ...
        this._browser.addCommand(
            ...
            this._enablePerformanceAudits.bind(this),
        )
        this._browser.addCommand(
            'disablePerformanceAudits',
            this._disablePerformanceAudits.bind(this),
        )
        this._browser.addCommand(
            'emulateDevice',
            this._emulateDevice.bind(this),
        )

        this._pwaGatherer = new PWAGatherer(
            this._session,
            this._page,
            this._driver,
        )
    ...
    private _propagateWSEvents(event: { data: string }) {
        ...
        log.debug(
            `cdp event: ${method} with params ${JSON.stringify(data.params)}`,
        )
...
type ServiceCommands = Omit<
    FunctionProperties<DevToolsService>,
    keyof Services.HookFunctions | '_setupHandler'
>
type CommandHandlerCommands = FunctionProperties<CommandHandler>
type AuditorCommands = Omit<
    FunctionProperties<Auditor>,
    '_audit' | '_auditPWA' | 'updateCommands'
>
...
export type BrowserExtensionSync = {
    [K in keyof BrowserExtension]: (
        ...args: Parameters<BrowserExtension[K]>
    ) => ThenArg<ReturnType<BrowserExtension[K]>>
}
