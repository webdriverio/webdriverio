...
test('beforeSession', () => {
    const service = new DevToolsService({}, [{}], {}, browser)
...
test('if not supported by browser', async () => {
    ...
    expect(browser.addCommand.mock.calls).toHaveLength(0)
})
...
test('if supported by browser', async () => {
    ...
    expect(browser.emit).toBeCalledTimes(1)
    expect(browser.emit).toBeCalledWith('foo', 'bar')
})
...
test('onReload hook', async () => {
    const service = new DevToolsService({}, [{}], {}, browser)
    service._setupHandler = jest.fn()
    browser.puppeteer = 'suppose to be reset after reload'
    service.onReload()
    expect(service._setupHandler).toBeCalledTimes(1)
    expect(browser.puppeteer).toBeNull()
})
