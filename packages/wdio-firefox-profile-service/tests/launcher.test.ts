...
describe('Firefox profile service', () => {
    describe('onPrepare', () => {
        ...
        test('should set preferences with no extensions - modern', async () => {
            ...
            expect(service['_profile']!.setPreference).toHaveBeenCalledWith(
            ...
            expect(capabilities[0]['moz:firefoxOptions']).toEqual({
                profile: 'foobar',
            })
        })
        ...
        test('should set preferences with extensions', async () => {
            ...
            expect(
                vi.mocked(service['_profile']!.addExtensions).mock.calls[0][0],
            ).toBe(options.extensions)
        })
        ...
        test('should set capabilities when in multiremote', async () => {
            ...
            expect(
                capabilities.firefox.capabilities['moz:firefoxOptions'],
            ).toEqual({ profile: 'foobar' })
        })
        ...
        test('should not set capabilities when an object and not firefox', async () => {
            ...
            expect(capabilities.foo.capabilities).not.toHaveProperty(
                'firefox_profile',
            )
            expect(capabilities.foo.capabilities).not.toHaveProperty(
                'moz:firefoxOptions',
        ...
        test('should set proxy', async () => {
            ...
            expect(service['_profile']!.setProxy).toHaveBeenCalledWith(
                options.proxy,
            )
        ...
        test('should load from directory if profileDirectory is set', async () => {
            ...
            expect(FirefoxProfile.copy).toHaveBeenCalledWith(
        ...
        test('should not continue if profile could not be created', async () => {
            ...
            vi.mocked(FirefoxProfile.copy).mockImplementationOnce(((
                _: string,
                cb: Function,
            ) => cb()) as any)
            const service = new Launcher(options)
