...
test('suiteStarted', () => {
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].uid).toBe(
        'some test suite23',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].title).toBe(
        'some test suite',
    )
...
test('specStarted', () => {
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][1].uid).toBe(
        'some test spec24',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][1].title).toBe(
        'some test spec',
    )
...
test('specDone', () => {
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].cid).toBe('0-2')
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].uid).toBe(
        'some failing test spec25',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].pending).toBe(false)
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].error.message).toBe(
        'foobar',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][0]).toBe('test:end')
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][1].uid).toBe(
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].cid).toBe('0-2')
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].uid).toBe(
        'some pending test spec26',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].pending).toBe(true)
    expect(vi.mocked(runnerReporter.emit).mock.calls[0][1].pendingReason).toBe(
        'for no reason',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][0]).toBe('test:end')
    expect(vi.mocked(runnerReporter.emit).mock.calls[1][1].uid).toBe(
...
test('specDone should pass multiple failed expectations as errors', () => {
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[2][1].error.message).toBe(
        'I failed',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[2][1].errors.length).toBe(
        2,
    )
    expect(
        vi.mocked(runnerReporter.emit).mock.calls[2][1].errors[0].message,
    ).toBe('I failed')
    expect(
        vi.mocked(runnerReporter.emit).mock.calls[2][1].errors[1].message,
    ).toBe('I failed too!')
})
...
test('suiteDone', () => {
    ...
    expect(vi.mocked(runnerReporter.emit).mock.calls[4][1].title).toBe(
        '<unknown test>',
    )
    expect(vi.mocked(runnerReporter.emit).mock.calls[5][0]).toBe('test:fail')
    expect(vi.mocked(runnerReporter.emit).mock.calls[5][1].error.message).toBe(
        'foobar',
    )
...
test('do not clean stack option', () => {
    ...
    const event = {
        id: '24',
        description: 'some test spec',
        failedExpectations: [error as any],
        status: 'failed',
        fullName: 'foobar',
    }
    const event2 = {
        id: '24',
        description: 'some test spec',
        failedExpectations: [error2 as any],
        status: 'failed',
        fullName: 'foobar',
    }
    const dirtyRunnerReporter: EventEmitter = { emit: vi.fn() } as any
    const dirtyJasmineReporter = new JasmineReporter(
        dirtyRunnerReporter,
        Object.assign({ cleanStack: false }, PARAMS),
    )
    jasmineReporter.specDone(event as any)
    dirtyJasmineReporter.specDone(event2 as any)
    expect(
        vi
            .mocked(dirtyRunnerReporter.emit)
            .mock.calls[0][1].error.stack.split('\n').length,
    ).toBeGreaterThan(
