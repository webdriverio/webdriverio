...
class JunitReporter extends WDIOReporter {
    ...
    private _addFailedHooks(suite: SuiteStats) {
        ...
        const failedHooks = suite.hooks.filter(
            (hook) =>
                hook.error &&
                hook.title.match(/^"(before|after)( all| each)?" hook/),
        )
        ...
        if (suite.type === 'feature') {
            ...
            const classNameFormat = this.options.classNameFormat
                ? this.options.classNameFormat({
                      ...
                      activeFeatureName: this._activeFeatureName,
                  })
                : `${this._packageName}.${this._activeFeatureName}`
            const testCase = this._activeFeature
                .testCase()
            ...
            for (let stepKey of Object.keys(scenario.tests)) {
                // tests are trested as steps in Cucumber
            ...
            if (test.state === 'pending' || test.state === 'skipped') {
                ...
                if (test.error) {
                    if (test.error.message) {
                        test.error.message = test.error.message.replace(
                            ansiRegex,
                            '',
                        )
                    }
                    ...
                    if (this.options.errorOptions) {
                        ...
                        for (const key of Object.keys(errorOptions)) {
                            testCase[key](
                                (test.error as any)[errorOptions[key]],
                            )
                        }
                    } else {
                        // default
    ...
    private _buildJunitXml(runner: RunnerStats) {
        ...
        if (
            ...
            this._packageName =
                this.options.packageName || browserstackSanitizedCapabilities
        } else {
            this._packageName =
                this.options.packageName || runner.sanitizedCapabilities
        }
                ...
                case 'command':
                    standardOutput.push(
                        data.method
                            ...
                            : `COMMAND: ${data.command} - ${this._format(
                                  data.params,
                              )}`,
                    )
                    break
