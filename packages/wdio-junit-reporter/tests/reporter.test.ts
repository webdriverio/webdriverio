...
describe('wdio-junit-reporter', () => {
    ...
    it('should prepare name', () => {
        ...
        expect(
            reporter['_prepareName']('Chrome something_odd.foobar'),
        ).toMatchSnapshot()
        expect(
            reporter['_prepareName']('Chrome @something_odd.foobar'),
        ).toMatchSnapshot()
    ...
    it('has a suiteNameFormat option for setting names manually', () => {
        reporter = new WDIOJunitReporter({
            stdout: true,
            suiteNameFormat: /foobar/,
        })
        expect(
            reporter['_prepareName']('Skipped_foobar_test'),
        ).toMatchSnapshot()
    })
    ...
    it('can format output', () => {
        ...
        expect(
            reporter['_format']({ a: 'foobar'.repeat(100) }),
        ).toMatchSnapshot()
        expect(reporter['_format']({ a: '@'.repeat(10000) })).toMatchSnapshot()
        expect(
            reporter['_format']({ a: '@'.repeat(100).split('') }),
        ).toMatchSnapshot()
        expect(
            reporter['_format'](
                [...Array(11)]
            ...
            '{"entry0":"foobar","entry1":"foobar","entry2":"foobar","entry3":"foobar","entry4":"foobar","entry5":"foobar","entry6":"foobar","entry7":"foobar","entry8":"foobar","entry9":"foobar","_":"1 more keys: [\\"entry10\\"]"}',
        )
    })
    ...
    it('can translate stdout', () => {
        ...
        expect(reporter['_getStandardOutput'](teststats)).toContain(
            'COMMAND: POST /sessionId/click - {"elementId":"foobar"}',
        )
        expect(reporter['_getStandardOutput'](teststats)).toContain(
            'RESULT: {"value":"foobar"}',
        )
    ...
    it('generates xml output', () => {
        ...
        expect(
            reporter['_buildJunitXml'](mochaRunnerLog as any).replace(
                /\s/g,
                '',
            ),
        ).toMatchSnapshot()
    ...
    it('generates xml output (Cucumber-style)', () => {
        ...
        // verifies the content of the report but omits format by stripping all whitespace and new lines
        expect(
            reporter['_buildJunitXml'](cucumberRunnerLog as any).replace(
                /\s/g,
                '',
            ),
        ).toMatchSnapshot()
    ...
    it('generates xml output with correct information when test is ran against Browserstack for iOS apps', () => {
        ...
        // verifies the content of the report but omits format by stripping all whitespace and new lines
        expect(
            reporter['_buildJunitXml'](
                cucumberRunnerBrowserstackIosLog as any,
            ).replace(/\s/g, ''),
        ).toMatchSnapshot()
    })
    ...
    it('generates xml output with correct information when test is ran against Browserstack for Android apps', () => {
        ...
        expect(
            reporter['_buildJunitXml'](
                cucumberRunnerBrowserstackAndroidLog as any,
            ).replace(/\s/g, ''),
        ).toMatchSnapshot()
    })
    ...
    it('ensures that capabilities passed to buildJunitXml are not null/undefined', () => {
        ...
        expect(
            reporter['_buildJunitXml'](
                cucumberRunnerBrowserstackAndroidLogMissingOS as any,
            ).replace(/\s/g, ''),
        ).toMatchSnapshot()
    })
    ...
    it('generates xml output correctly when having classNameFormat override with mocha', () => {
        reporter = new WDIOJunitReporter({
            stdout: true,
            classNameFormat: ({ packageName, suite }) =>
                `foo-${packageName}-${suite!.fullTitle}`,
        })
        reporter.suites = suitesErrorLog as any
        expect(
            reporter['_buildJunitXml'](mochaRunnerLog as any).replace(
    ...
    it('generates xml output correctly when having classNameFormat override with cucumber', () => {
        reporter = new WDIOJunitReporter({
            stdout: true,
            classNameFormat: ({ packageName, activeFeatureName }) =>
                `foo-${packageName}-${activeFeatureName}`,
        })
        reporter.suites = featuresLog as any
    ...
    it('generates xml output correctly when having testSuiteNameFormat override with mocha', () => {
        reporter = new WDIOJunitReporter({
            stdout: true,
            suiteNameFormat: ({ name, suite }) => `foo ${name} ${suite.title}`,
        })
        reporter.suites = suitesErrorLog as any
        expect(
            reporter['_buildJunitXml'](mochaRunnerLog as any).replace(
