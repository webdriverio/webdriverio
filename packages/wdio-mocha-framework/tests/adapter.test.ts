...
const adapterFactory = (config: any) =>
    new MochaAdapter(
        '0-2',
        { ...config },
        ['/foo/bar.test.js'],
...
test('options', () => {
    ...
    expect(adapter.requireExternalModules).toBeCalledWith([
        'the/compiler.js',
        'foo/bar.js',
    ])
})
...
describe('preRequire', () => {
    test('preRequire - TDD', () => {
        ...
        const adapter = adapterFactory({
            mochaOpts,
            beforeHook: 'beforeHook123',
            afterHook: 'afterHook123',
            beforeTest: 'beforeTest234',
            afterTest: 'afterTest234',
        })
        adapter.preRequire()
        ...
        expect(
            hookArgsFn({ test: { foo: 'bar', parent: { title: 'parent' } } }),
        ).toEqual([
            { foo: 'bar', parent: 'parent' },
            { test: { foo: 'bar', parent: { title: 'parent' } } },
        ])
    })
...
test('wrapHook if successful', async () => {
    ...
    expect(
        (vi.mocked(executeHooksWithArgs).mock.calls[0][2] as any)[0].type,
    ).toBe('beforeAll')
})
...
describe('prepareMessage', () => {
    test('should prepare a message', async () => {
        ...
        expect(
            result.duration! >= 5000 && result.duration! <= 5020,
        ).toBeTruthy()
    })
...
test('requireExternalModules', () => {
    ...
    expect(loadModule).toBeCalledWith(
        path.resolve(__dirname, '..', '..', '..', 'bar', 'foo.js'),
    )
})
...
test('emit properly reports to reporter', () => {
    ...
    const adapter = adapterFactory()
    adapter.getUID = () => '123'
    adapter.emit('suite:start', { title: 'foobar' }, new Error('uups') as any)
...
describe('loadFiles', () => {
    ...
    test('should set _hasTests to false if there no tests to run', async () => {
        ...
        expect(
            vi.mocked(Mocha.Runner).mock.results[0].value.grep,
        ).toBeCalledWith('regexp foo', 'invert')
    ...
    test('should propagate error', async () => {
        ...
        expect(adapter['_specLoadError']!.message).toContain(
