...
export default class Runner extends EventEmitter {
    ...
    async run({ cid, args, specs, caps, configFile, retries }: RunParams) {
        ...
        this._specFileRetryAttempts =
            (this._config.specFileRetries || 0) - (retries || 0)
        logger.setLogLevelsConfig(this._config.logLevels, this._config.logLevel)
        const isMultiremote = (this._isMultiremote = !Array.isArray(
            this._configParser.getCapabilities(),
        ))
        ...
        const beforeSessionParams: BeforeSessionArgs = [
            this._config,
            this._caps,
            this._specs,
            this._cid,
         ...
         */
        this._framework = await this.#initFramework(
            cid,
            this._config,
            caps,
            this._reporter,
        ...
        process.send!({
            ...
            content: { cid, caps, specs, hasTests: this._framework.hasTests() },
        })
        if (!this._framework.hasTests()) {
            return this._shutdown(0, retries, true)
        }

        browser = await this._initSession(
            this._config as SingleConfigOption,
            this._caps,
            browser,
        )
        ...
        this._reporter.caps =
            browser.capabilities as Capabilities.RemoteCapability
        ...
        if (this._sigintWasCalled) {
            log.info(
                'SIGINT signal detected while starting session, shutting down...',
            )
        ...
        const multiRemoteBrowser =
            browser as unknown as MultiRemoteBrowser<'async'>
        this._reporter.emit('runner:start', {
            ...
            instanceOptions: isMultiremote
                ? multiRemoteBrowser.instances.reduce(
                      (prev: any, browserName: string) => {
                          prev[multiRemoteBrowser[browserName].sessionId] =
                              multiRemoteBrowser[browserName]
                                  .options as Options.WebdriverIO
                          return prev
                      },
            ...
            capabilities: isMultiremote
                ? multiRemoteBrowser.instances.reduce(
                      (caps: any, browserName: string) => {
                          caps[browserName] =
                              multiRemoteBrowser[browserName].capabilities
                          caps[browserName].sessionId =
        ...
        try {
            ...
            await this._fetchDriverLogs(
                this._config,
                (caps as Required<Capabilities.DesiredCapabilities>)
                    .excludeDriverLogs,
            )
        } catch (err: any) {
            log.error(err)
    ...
    async #initFramework(
        ...
        const runner = Array.isArray(config.runner)
            ? config.runner[0]
            : config.runner
        ...
        if (runner === 'local') {
            const framework = (
                await initialisePlugin(config.framework as string, 'framework')
    ...
    private async _initSession(
        ...
        if (browserStub) {
            Object.entries(browserStub).forEach(
                ([key, value]: [keyof Browser<'async'>, any]) => {
        ...
        _setGlobal(
            '$',
            (selector: Selector) => browser.$(selector),
            config.injectGlobals,
        )
        _setGlobal(
            '$$',
            (selector: Selector) => browser.$$(selector),
    ...
    private async _startSession(
        ...
        try {
            this._browser = await initialiseInstance(
                config,
                caps,
                this._isMultiremote,
            )
        ...
        return Promise.all(
            logTypes.map(async (logType) => {
                ...
                try {
                    ...
                    return log.warn(
                        `Couldn't fetch logs for ${logType}: ${e.message}`,
                    )
                }
                ...
                const stringLogs = logs
                    .map((log: any) => JSON.stringify(log))
                    .join('\n')
                return fs.writeFile(
                    path.join(
                        config.outputDir!,
                        `wdio-${this._cid}-${logType}.log`,
                    ),
                    stringLogs,
                    'utf-8',
                )
    ...
    async endSession() {
        ...
        const capabilities:
            ...
            | MultiRemoteCaps = this._browser?.capabilities || {}
        if (this._isMultiremote) {
            multiremoteBrowser.instances.forEach((browserName: string) => {
                ;(capabilities as MultiRemoteCaps)[browserName] =
                    multiremoteBrowser[browserName].capabilities
            })
        }
        ...
        const afterSessionArgs: AfterSessionArgs = [
            this._config!,
            capabilities,
            this._specs as string[],
        ]
        await executeHooksWithArgs(
            'afterSession',
