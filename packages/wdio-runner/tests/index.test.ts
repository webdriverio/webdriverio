...
describe('wdio-runner', () => {
    ...
    describe('_fetchDriverLogs', () => {
        ...
        it('not do anything if driver does not support log commands', async () => {
            ...
            const result = await runner['_fetchDriverLogs'](
                { outputDir: '/foo/bar', capabilities: {} },
                ['*'],
            )
        ...
        it('should not write to file excluded logTypes', async () => {
            ...
            expect(vi.mocked(fs.writeFile).mock.calls[0][0]).toMatch(
                /(\\|\/)foo(\\|\/)bar(\\|\/)wdio-0-1-foo.log/,
            )
            expect(vi.mocked(fs.writeFile).mock.calls[0][1]).toEqual(
                '{"message":"#1 foo log"}\n{"message":"#2 foo log"}',
            )
            expect(vi.mocked(fs.writeFile).mock.calls[0][2]).toEqual('utf-8')
        })
        ...
        it('should fetch logs', async () => {
            ...
            expect(vi.mocked(fs.writeFile).mock.calls[1][0]).toMatch(
                /(\\|\/)foo(\\|\/)bar(\\|\/)wdio-0-1-bar.log/,
            )
            expect(vi.mocked(fs.writeFile).mock.calls[0][1]).toEqual(
                '{"message":"#1 foo log"}\n{"message":"#2 foo log"}',
            )
    ...
    describe('endSession', () => {
        ...
        it('should work normally when called after framework run in multiremote', async () => {
            ...
            expect(executeHooksWithArgs).toBeCalledWith(
                ...
                [hook],
                [
                    { logLevel: 'error', afterSession: [hook] },
            ...
            expect(runner['_browser']!.deleteSession).toBeCalledTimes(1)
            expect(
                !(runner['_browser'] as any as WebdriverIO.MultiRemoteBrowser)
                    .foo.sessionId,
            ).toBe(true)
            expect(
                !(runner['_browser'] as any as WebdriverIO.MultiRemoteBrowser)
    ...
    describe('run', () => {
        ...
        it('should auto compile if args are given', async () => {
            ...
            const runner = new WDIORunner()
            runner['_shutdown'] = vi.fn()
            vi.spyOn(ConfigParser.prototype, 'getConfig').mockReturnValue(
                config,
            )
            await runner.run({
                configFile: '/foo/bar',
        ...
        it('should return failures count', async () => {
            ...
            runner['_initSession'] = vi
                .fn()
                .mockReturnValue({ options: { capabilities: {} } })
        ...
        it('should set failures to 1 in case of error', async () => {
            ...
            expect(vi.mocked(runner.emit).mock.calls[0]).toEqual([
                'error',
                new Error('framework testThrows failed'),
            ])
        })
        ...
        it('should shutdown if session was not created', async () => {
            ...
            expect(executeHooksWithArgs).toBeCalledWith('after', 'foobar', [
                1,
                caps,
                specs,
            ])
    ...
    describe('_initSession', () => {
        it('should register browser to global scope', async () => {
            ...
            expect(_setGlobal).toBeCalledWith(
                'expect',
                expect.any(Function),
                undefined,
            )
        ...
        it('should register before and after command listener', async () => {
            ...
            expect(reporter.emit).toBeCalledWith('client:beforeCommand', {
                foo: 'bar',
                sessionId: 'fakeid',
            })
            ...
            expect(reporter.emit).toBeCalledWith('client:afterCommand', {
                bar: 'foo',
                sessionId: 'fakeid',
            })
        })
    ...
    describe('_shutdown', () => {
        ...
        it('should emit runner:start if the initialisation failed', async () => {
            const runner = new WDIORunner()
            runner['_configParser'] = {
                getCapabilities: vi
                    .fn()
                    .mockReturnValue([{ browserName: 'safari' }]),
            }
            runner['_browser'] = {} as any as BrowserObject
            runner['_reporter'] = {
                waitForSync: vi.fn().mockReturnValue(Promise.resolve()),
