...
describe('BaseReporter', () => {
    ...
    it('getLogFile', async () => {
        ...
        expect(reporter.getLogFile('foobar')).toMatch(
            /(\\|\/)foo(\\|\/)bar(\\|\/)wdio-0-0-foobar-reporter.log/,
        )
    })
    ...
    it('can set custom logFile property', async () => {
        ...
        // @ts-expect-error
        expect(reporter['_reporters'][0].options.logFile).toBe('/barfoo.log')
        // @ts-expect-error
        expect(reporter['_reporters'][1].options.logFile).toBe('/foobar.log')
    })
    ...
    it('should output log file to custom outputDir', async () => {
        ...
        await reporter.initReporters()
        expect(reporter.getLogFile('dot')).toMatch(
            /(\\|\/)foo(\\|\/)bar(\\|\/)baz(\\|\/)wdio-0-0-dot-reporter.log/,
        )
    })
    ...
    it('should return custom log file name using cid and capabilities', async () => {
        ...
        await reporter.initReporters()
        expect(reporter.getLogFile('dot')).toMatch(
            /(\\|\/)foo(\\|\/)bar(\\|\/)wdio-results-0-0-foo.log/,
        )
    })
    ...
    it('should emit events to all reporters', async () => {
        ...
        expect(
            reporter['_reporters'].map((r) => vi.mocked(r.emit).mock.calls),
        ).toEqual([
    ...
    it('should send printFailureMessage', async () => {
        ...
        expect(vi.mocked(process.send)!.mock.calls[0][0].name).toBe(
            'printFailureMessage',
        )
    })
    ...
    it('should allow to write to output directory with custom reporter', async () => {
        ...
        expect(reporter.getLogFile('CustomReporter')).toMatch(
            /(\\|\/)foo(\\|\/)baz(\\|\/)bar(\\|\/)wdio-0-0-CustomReporter-reporter.log/,
        )
    })
    ...
    it('it should fail if waitForSync times out', async () => {
        ...
        await expect(reporter.waitForSync()).rejects.toEqual(
            new Error('Some reporters are still unsynced: CustomReporter'),
        )
    })
