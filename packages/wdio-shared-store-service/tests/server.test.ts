...
describe('WdioSharedStoreService exports', () => {
    ...
    it('should handle non json type', async () => {
        const response = await got.post(getUrl, {
            body: 'foobar',
    ...
    it('should handle 404', async () => {
        const response = await got.post(getUrl + 'foobar', {
            throwHttpErrors: false,
        })
    ...
    describe('setting/getting entries', () => {
        ...
        it('should get entry', async () => {
            ...
            const res = await got.post<{ value: string }>(getUrl, {
                json: { key: 'foobar' },
                responseType: 'json',
            })
    ...
    afterEach(() => {
        Object.keys(__store).forEach((key) => {
            delete __store[key]
        })
