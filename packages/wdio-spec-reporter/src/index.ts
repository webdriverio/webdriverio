...
export default class SpecReporter extends WDIOReporter {
    ...
    constructor(options: SpecReporterOptions) {
        ...
        this._sauceLabsSharableLinks =
            'sauceLabsSharableLinks' in options
    ...
    onRunnerStart(runner: RunnerStats) {
        this._preface = this._showPreface
    ...
    printCurrentStats(stat: TestStats | HookStats | SuiteStats) {
        ...
        const suiteStartBanner =
            stat.type === 'feature' ||
            stat.type === 'suite' ||
            stat.type === 'suite:start'
    ...
    printReport(runner: RunnerStats) {
        ...
        const testLinks = runner.isMultiremote
            ? Object.entries(runner.capabilities)
                .map(
                    ([instanceName, capabilities]: [
                          string,
                          CapabilitiesWithSessionId
                      ]) =>
                        this.getTestLink({
                            capabilities,
        ...
        const prefacedOutput = this._showPreface
            ? output.map((value) => {
    ...
    getTestLink({
        ...
        const config =
            this.runnerStat && this.runnerStat.instanceOptions[sessionId]
        ...
        if (isSauceJob && config && config.user && config.key && sessionId) {
            ...
            if ('testobject_test_report_url' in capabilities) {
                return [
                    format(
                        note,
                        multiremoteNote,
                        capabilities.testobject_test_report_url
                    ),
                ]
            }
            ...
            const isUSEast1 =
                config.headless || config.hostname?.includes('us-east-1')
            const isUSEast4 =
                ['us-east-4'].includes(config?.region || '') ||
                config.hostname?.includes('us-east-4')
            const isEUCentral =
                ['eu', 'eu-central-1'].includes(config?.region || '') ||
                config.hostname?.includes('eu-central')
            const isAPAC =
                ['apac', 'apac-southeast-1'].includes(config?.region || '') ||
                config.hostname?.includes('apac')
            const dc = isUSEast1
                ? '.us-east-1'
                : isUSEast4
                    ? '.us-east-4'
    ...
    getHeaderDisplay(runner: RunnerStats) {
        const combo = this.getEnviromentCombo(
            runner.capabilities,
            undefined,
            runner.isMultiremote
        ).trim()
    ...
    getEventsToReport(suite: SuiteStats) {
        return [
            ...
            ...suite.hooksAndTests
                .reduce(
                    (
                        accumulator: (HookStats | TestStats)[],
                        currentItem: HookStats | TestStats
                    ) => {
                        if (currentItem instanceof TestStats) {
                            const existingTestIndex = accumulator.findIndex(
                                (test: HookStats | TestStats) =>
                                    test instanceof TestStats
                            )
                            const existingTest = accumulator[
                                existingTestIndex
                            ] as TestStats
                            if (
                                currentItem.retries &&
                                existingTest.retries &&
                                currentItem.retries > existingTest.retries
                            ) {
                                accumulator.splice(
                                    existingTestIndex,
                                    1,
                    ...
                    [] as (HookStats | TestStats)[]
                )
                .filter(
                    (item: HookStats | TestStats) =>
                        Object.keys(item).length > 0
                )
                .filter((item: HookStats | TestStats) => {
    ...
    getResultDisplay(prefaceString?: string) {
        ...
        for (const suite of suites) {
            ...
            if (suite.file && !specFileReferences.includes(suite.file)) {
                output.push(
                    `${suiteIndent}» ${suite.file.replace(process.cwd(), '')}`
                )
            ...
            for (const test of eventsToReport) {
                ...
                output.push(
                    `${testIndent}${chalk[this.getColor(state)](
                        this.getSymbol(state)
                    )} ${testTitle.trim()}`
                )
                ...
                if (typeof arg === 'string') {
                    ...
                    const formattedDocstringLines = docstring
                        .split('\n')
                        .filter((line) => line)
                        .map(
                            (line: string) =>
                                `${testIndent}${docstringIndent}${line}`
                        )
                    output.push(
                        ...[
                            docstringMark,
                            ...formattedDocstringLines,
                            docstringMark,
                        ]
                    )
                } else {
                ...
                if (logItem) {
                    output.push('')
                    output.push(
                        testIndent.repeat(2) + '.........Console Logs.........'
                    )
                    output.push(
                        testIndent.repeat(3) +
                            logItem?.replace(
    ...
    getCountDisplay(duration: string) {
        ...
        if (this._stateCounts.failed > 0) {
            const text =
                `${this._stateCounts.failed} failing ${duration}`.trim()
        ...
        if (this._stateCounts.skipped > 0) {
            const text =
                `${this._stateCounts.skipped} skipped ${duration}`.trim()
    ...
    getFailureDisplay() {
        ...
        for (const suite of suites) {
            ...
            for (const test of eventsToReport) {
                ...
                const testTitle = test.title
                const errors: Error[] =
                    test.errors || (test.error ? [test.error] : [])
    ...
    getOrderedSuites() {
        ...
        if (rootSuite) {
            ...
            const beforeAllHooks = rootSuite.hooks.filter(
                (hook) =>
                    hook.state &&
                    hook.title.startsWith('"before') &&
                    hook.title.endsWith('"{root}"')
            )
            const afterAllHooks = rootSuite.hooks.filter(
                (hook) =>
                    hook.state &&
                    hook.title.startsWith('"after') &&
                    hook.title.endsWith('"{root}"')
            )
            this._orderedSuites.unshift(
                Object.assign({} as SuiteStats, baseRootSuite, {
                    hooks: beforeAllHooks,
                    hooksAndTests: beforeAllHooks,
                })
            )
            this._orderedSuites.push(
                Object.assign({} as SuiteStats, baseRootSuite, {
                    hooks: afterAllHooks,
    ...
    getEnviromentCombo(
        ...
        const app = (caps['appium:app'] || (caps as any).app || '').replace(
            'sauce-storage:',
            ''
        )
        const appName =
            app || caps['appium:bundleId'] || (caps as any).bundleId
