...
export default function WebDriver(
    ...
    function unit(
        ...
        eventMiddleware?: { socket: Partial<EventEmitter> },
    ) {
        if (eventMiddleware) {
        ...
        if (typeof commandWrapper === 'function') {
            for (const [commandName, { value }] of Object.entries(
                ...
                propertiesObject[commandName].value = commandWrapper(
                    commandName,
                    value,
                    propertiesObject,
                )
        ...
        const { puppeteer, ...propertiesObjectWithoutPuppeteer } =
            propertiesObject
        ...
        client.addCommand = function (
            name: string,
            func: Function,
            attachToElement = false,
            proto: Record<string, any>,
            instances?: Clients.Multiremote | Clients.Browser,
        ) {
            const customCommand =
                typeof commandWrapper === 'function'
         ...
         */
        client.overwriteCommand = function (
            name: string,
            func: Function,
            attachToElement = false,
            proto: Record<string, any>,
            ...
            if (attachToElement) {
                if (instances) {
                     ...
                     */
                    Object.values(instances).forEach((instance) => {
                        instance.__propertiesObject__.__elementOverrides__.value[
                            name
                        ] = customCommand
                    })
                } else {
                     ...
                     * regular mode
                     */
                    this.__propertiesObject__.__elementOverrides__.value[name] =
                        customCommand
                }
            } else if (client[name]) {
                const origCommand = client[name]
                delete client[name]
                unit.lift(name, customCommand, proto, (...args: any[]) =>
                    origCommand.apply(this, args),
                )
            } else {
                throw new Error(
                    'overwriteCommand: no command to be overwritten: ' + name,
                )
            }
        }
    ...
    unit.lift = function (
        ...
        ;(proto || prototype)[name] = function next(...args: any[]) {
            ...
            const result = func.apply(
                this,
                origCommand ? [origCommand, ...args] : args,
            )
    ...
    for (let eventCommand in EVENTHANDLER_FUNCTIONS) {
        prototype[eventCommand] = function (...args: [any, any]) {
            eventHandler[eventCommand as keyof EventEmitter](
                ...(args as [never, any]),
            )

            if (prototype.eventMiddleware) {
                if (
                    typeof prototype.eventMiddleware[
                        eventCommand as keyof EventEmitter
                    ] === 'function'
                ) {
