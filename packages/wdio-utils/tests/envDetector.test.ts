...
describe('sessionEnvironmentDetector', () => {
    ...
    const appiumCaps = appiumResponse.value
        .capabilities as WebDriver.Capabilities
    const experitestAppiumCaps = experitestResponse.appium
        .capabilities as WebDriver.Capabilities
    const geckoCaps = geckodriverResponse.value
        .capabilities as WebDriver.Capabilities
    const edgeCaps = edgedriverResponse.value
        .capabilities as WebDriver.Capabilities
    const phantomCaps = ghostdriverResponse.value as WebDriver.Capabilities
    const safariCaps = safaridriverResponse.value
        .capabilities as WebDriver.Capabilities
    const safariDockerNpNCaps = safaridriverdockerNpNResponse.value
        .capabilities as WebDriver.Capabilities // absent capability.platformName
    const safariDockerNbVCaps = safaridriverdockerNbVResponse.value
        .capabilities as WebDriver.Capabilities // absent capability.browserVersion
    const safariLegacyCaps =
        safaridriverLegacyResponse.value as WebDriver.Capabilities
    const standaloneCaps =
        seleniumstandaloneResponse.value as WebDriver.DesiredCapabilities
    const standalonev4Caps =
        seleniumstandalone4Response.value as WebDriver.DesiredCapabilities

    it('isMobile', () => {
        const requestedCapabilities = { browserName: '' }
        const appiumReqCaps = { 'appium:options': {} }
        const appiumW3CCaps = {
            alwaysMatch: { 'appium:options': {} },
            firstMatch: [],
        }
        expect(
            sessionEnvironmentDetector({
        ...
        expect(
            sessionEnvironmentDetector({
                capabilities: experitestAppiumCaps,
                requestedCapabilities,
            }).isMobile,
        ).toBe(true)
        expect(
        ...
        expect(
            sessionEnvironmentDetector({
                capabilities: chromeCaps,
                requestedCapabilities: appiumReqCaps,
            }).isMobile,
        ).toBe(false)
        expect(
            sessionEnvironmentDetector({
                capabilities: chromeCaps,
                requestedCapabilities: appiumW3CCaps,
            }).isMobile,
        ).toBe(false)
    ...
    it('isW3C', () => {
        ...
        expect(
            sessionEnvironmentDetector({
                capabilities: safariDockerNbVCaps,
                requestedCapabilities,
            }).isW3C,
        ).toBe(true)
        expect(
            sessionEnvironmentDetector({
                capabilities: safariDockerNpNCaps,
                requestedCapabilities,
            }).isW3C,
        ).toBe(true)
        expect(
            sessionEnvironmentDetector({
                capabilities: edgeCaps,
                requestedCapabilities,
            }).isW3C,
        ).toBe(true)
        expect(
            sessionEnvironmentDetector({
                capabilities: safariLegacyCaps,
                requestedCapabilities,
            }).isW3C,
        ).toBe(false)
        expect(
    ...
    it('isSauce (w3c)', () => {
        ...
        requestedCapabilities.alwaysMatch = {
            'sauce:options': { extendedDebugging: true },
        }
        expect(
            sessionEnvironmentDetector({ capabilities, requestedCapabilities })
                .isSauce,
    ...
    it('isSeleniumStandalone', () => {
        ...
        expect(
            sessionEnvironmentDetector({
                capabilities: standaloneCaps,
                requestedCapabilities,
            }).isSeleniumStandalone,
        ).toBe(true)
        expect(
            sessionEnvironmentDetector({
                capabilities: standalonev4Caps,
                requestedCapabilities,
            }).isSeleniumStandalone,
        ).toBe(true)
    })
    ...
    it('should not detect mobile app for browserName===undefined', function () {
        const requestedCapabilities = { browserName: '' }
        const capabilities = {}
        const { isMobile, isIOS, isAndroid } = sessionEnvironmentDetector({
            capabilities,
            requestedCapabilities,
        })
...
describe('capabilitiesEnvironmentDetector', () => {
    it('should return env flags without isW3C and isSeleniumStandalone', () => {
        const sessionFlags = sessionEnvironmentDetector({
            capabilities: {},
        ...
        const expectedFlags = Object.keys(sessionFlags).filter(
            (flagName) => !['isW3C', 'isSeleniumStandalone'].includes(flagName),
        )
