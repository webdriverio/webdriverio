...
beforeEach(() => {
    prototype = {
        ...
        someFunc: {
            value: vi
                .fn()
                .mockImplementation((arg) => `result-${arg.toString()}`),
        },
    }
})
...
describe('monad', () => {
    ...
    it('should throw if there is no command to be overwritten', () => {
        ...
        expect(() => client.overwriteCommand('someFunc2', fn)).toThrow(
            'overwriteCommand: no command to be overwritten: someFunc2',
        )
    })
    ...
    it('should add element commands to the __propertiesObject__ cache', () => {
        ...
        expect(typeof client.__propertiesObject__.myCustomElementCommand).toBe(
            'object',
        )
        expect(client.__propertiesObject__.myCustomElementCommand.value).toBe(
            func,
        )
    })
    ...
    it('should add element commands for override to the __propertiesObject__.__elementOverrides__ cache', () => {
        ...
        expect(
            client.__propertiesObject__.__elementOverrides__.value.someFunc(
                2,
                3,
            ),
        ).toBe(5)
    ...
    it('should add element commands to the __propertiesObject__ cache in multiremote', () => {
        ...
        const instances = {
            foo: {
                __propertiesObject__: {
                    myCustomElementCommand: { value: undefined },
                },
            },
        }
        ...
        expect(
            typeof instances.foo.__propertiesObject__.myCustomElementCommand,
        ).toBe('object')
        expect(
            instances.foo.__propertiesObject__.myCustomElementCommand.value,
        ).toBe(func)
    })
    ...
    it('should add element commands for override to the __propertiesObject__.__elementOverrides__ cache in multiremote', () => {
        ...
        expect(
            instances.foo.__propertiesObject__.__elementOverrides__.value.someFunc(
                4,
                5,
            ),
        ).toBe(9)
    ...
    it('allows to use custom command wrapper', () => {
        ...
        const client = monad(
            sessionId,
            (commandName: string, commandFn: Function) => {
                return (...args: any[]) => {
                    return `${commandName}(${args.join(', ')}) = ${commandFn(
                        ...args,
                    )}`
                }
            },
