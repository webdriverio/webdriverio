...
import {
    filterStackTrace,
    testFnWrapper,
} from '../../src/test-framework/testFnWrapper.js'

vi.mock('../../src/shim', () => ({
    executeHooksWithArgs: vi.fn(),
    executeAsync: async (fn: Function, { limit, attempts }: any, args = []) =>
        fn('async', limit, attempts, ...args),
    runSync: null,
}))
...
describe('testFnWrapper', () => {
    const origFn = (mode: string, limit: number, attempts: number, arg: any) =>
        `${mode}: Foo${arg} ${limit} ${attempts}`
    const buildArgs = (
        specFn: Function,
        retries?: number,
        beforeFnArgs?: Function,
        afterFnArgs?: Function,
    ) => [
        'Foo',
    ...
    it('should throw on error', async () => {
        let expectedError
        const args = buildArgs(
            (mode: string, repeatTest: boolean, arg: any) => {
