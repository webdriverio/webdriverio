...
describe('utils', () => {
    it('commandCallStructure', () => {
        ...
        expect(
            ...
            'foobar("param", 1, true, <object>, <fn>, <fn>, <fn>, null, undefined, "<Screenshot[base64]>")',
        )
        expect(commandCallStructure('foobar', ['/html/body/a'])).toBe(
            'foobar("<Screenshot[base64]>")',
        )
        expect(commandCallStructure('findElement', ['/html/body/a'])).toBe(
            'findElement("/html/body/a")',
        )
        expect(commandCallStructure('findElements', ['/html/body/a'])).toBe(
            'findElements("/html/body/a")',
        )
        expect(
            commandCallStructure('findElementFromElement', ['/html/body/a']),
        ).toBe('findElementFromElement("/html/body/a")')
        expect(
            commandCallStructure('findElementsFromElement', ['/html/body/a']),
        ).toBe('findElementsFromElement("/html/body/a")')
    })

    it('transformCommandLogResult', () => {
        expect(transformCommandLogResult({ file: 'bar' })).toEqual({
            file: 'bar',
        })
        expect(
            transformCommandLogResult({
                file: Buffer.from('some screenshot').toString('base64'),
            }),
        ).toBe('"<Screenshot[base64]>"')

        expect(transformCommandLogResult({ script: 'foo' })).toEqual({
            script: 'foo',
        })
        expect(
            transformCommandLogResult({
                script: Buffer.from('some script payload').toString('base64'),
            }),
        ).toBe('"<Script[base64]>"')

        expect(transformCommandLogResult({ script: 'return foobar' })).toEqual({
        ...
        expect(
            transformCommandLogResult({
                script: '!function(t,e){"object"==typeof exports&&"object"==typeof mod...',
            }),
    ...
    describe('overwriteElementCommands', () => {
        it('should overwrite command', function () {
            ...
            expect((origFnMock as MockedFunction<any>).mock.calls.length).toBe(
                1,
            )
            expect((origFnMock as MockedFunction<any>).mock.instances[0]).toBe(
                propertiesObject.foo,
            )
        })
        ...
        it('should support rebinding when invoking original fn', function () {
            ...
            const propertiesObject = {
                ...
                __elementOverrides__: {
                    value: {
                        foo(origCmd: Function, arg: any) {
                            return [origCmd.call(origFnContext), arg]
                        },
                    },
                },
            }
        ...
        it('should throw if user command is not a function', function () {
            ...
            expect(() =>
                ...
                'overwriteCommand: commands be overwritten only with functions, command: foo',
            )
        })
        ...
        it('should throw on attempt to overwrite not a function', function () {
            ...
            expect(() =>
                ...
                'overwriteCommand: only functions can be overwritten, command: foo',
            )
        })
    })
    ...
    it('isValidParameter', () => {
        ...
        expect(isValidParameter([[]], '(number|string|object)[]')).toBe(true)
        expect(isValidParameter([null], '(number|string|object)[]')).toBe(false)
        expect(isValidParameter([false], '(number|string|object)[]')).toBe(
            false,
        )
        expect(isValidParameter(['1', false], '(number|string|object)[]')).toBe(
            false,
        )
    })
