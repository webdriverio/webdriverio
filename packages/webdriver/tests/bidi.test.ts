...
describe('BidiHandler', () => {
    ...
    describe('send', () => {
        it('fails if sending a message while not connected', async () => {
            const handler = new BidiHandler('ws://foo/bar')
            await expect(async () =>
                handler.send({ method: 'session.new', params: {} }),
        ...
        it('sends and waits for result', async () => {
            ...
            expect(handler.socket.on).toBeCalledWith(
                'message',
                expect.any(Function),
            )
            const [, messageCallback] = vi.mocked(handler.socket.on).mock
                .calls[0]

            messageCallback.call(this as any, Buffer.from('{somewrongmessage'))
            messageCallback.call(
                this as any,
                Buffer.from(JSON.stringify({ id: 1, result: 'foobar' })),
            )
    ...
    describe('sendAsync', () => {
        it('fails if sending a message while not connected', async () => {
            const handler = new BidiHandler('ws://foo/bar')
            await expect(async () =>
                handler.sendAsync({ method: 'session.new', params: {} }),
            ).rejects.toMatchSnapshot()
        })
