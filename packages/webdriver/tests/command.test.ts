...
describe('command wrapper', () => {
    it('should fail if wrong arguments are passed in', async () => {
        const commandFn = commandWrapper(
            commandMethod,
            commandPath,
            commandEndpoint,
        )
        await expect(commandFn).rejects.toThrow(
    ...
    it('should fail if arguments are malformed', async () => {
        ...
        await expect(() =>
            commandFn.call(scope, '123', 123, '123'),
        ).rejects.toThrow(/Malformed type for "using" parameter of command/)
    })
    ...
    it('should fail if not required param has wrong type', async () => {
        ...
        await expect(() =>
            commandFn.call(scope, '123', '123', '123', 'foobar'),
        ).rejects.toThrow(
    ...
    it('should throw if param type within array is not met', async () => {
        ...
        ).rejects.toThrow(/Actual: \(function\)\[\]/)
    })
...
describe('Bidi support', () => {
    it('fails if bidi is not supported', async () => {
        ...
        await expect(async () => commandFn.call(scope)).rejects.toThrow(
            /doesn't support WebDriver Bidi/,
        )
    })
    ...
    it('it propagates command to middleware', async () => {
        ...
        expect((scope as any).eventMiddleware.send).toHaveBeenCalledWith({
            params: 'foobar',
        })
    })
})
...
describe('command wrapper result log', () => {
    async function getRequestCallback(
        method: string,
        path: string,
        endpoint: Protocols.CommandEndpoint,
    ) {
