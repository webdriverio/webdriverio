...
describe('webdriver request', () => {
    ...
    it('should be able to make request', async () => {
        const req = new WebDriverRequest('POST', '/foo/bar', { foo: 'bar' })
        req['_createOptions'] = vi
            .fn()
            .mockImplementation((opts, sessionId) => ({
                foo: 'bar',
                sessionId,
            }))
        req.emit = vi.fn()
        ...
        await req.makeRequest(
            { connectionRetryCount: 43, logLevel: 'warn' },
            'some_id',
        )
    ...
    it('should pick up the fullRequestOptions returned by transformRequest', async () => {
        ...
        const transformRequest = vi
            .fn()
            .mockImplementation((requestOptions) => ({
                ...requestOptions,
                json: { foo: 'baz' },
            }))
    ...
    it('should resolve with the body returned by transformResponse', async () => {
        ...
        expect(transformResponse.mock.calls[0][1].json).toEqual({
            foo: 'requestBody',
        })
        await expect(responseBody).toEqual({
            value: { foo: 'transformedResponse' },
        })
        vi.mocked(got).mockClear()
    })
    ...
    describe('createOptions', () => {
        it('fails if command requires sessionId but none given', async () => {
            const req = new WebDriverRequest(
                'POST',
                `${webdriverPath}/:sessionId/element`,
                {},
            )
            await expect(() =>
                req['_createOptions']({ logLevel: 'warn' }),
            ).rejects.toThrow()
        })
        ...
        it('creates proper options set', async () => {
            ...
            const options: Options.RequestLibOptions = await req[
                '_createOptions'
            ](
                {
                    protocol: 'https',
            ...
            // @ts-ignore
            expect((options.agent! as gotOrig.Agents).https!.protocol).toBe(
                'https:',
            )
            // @ts-ignore
            expect((options.agent! as gotOrig.Agents).http!.protocol).toBe(
                'http:',
            )
            expect((options.url! as URL).href).toBe(
                'https://localhost:4445/session/foobar12345/element',
            )
            expect(
        ...
        it('ignors path when command is a hub command', async () => {
            ...
            expect((options.url as URL).href).toBe(
                'https://localhost:4445/grid/api/hub',
            )
        })
    ...
    describe('_request', () => {
        it('should make a request', async () => {
            ...
            expect(req.emit).toHaveBeenNthCalledWith(1, 'response', {
                result: expectedResponse,
            })
            expect(req.emit).toHaveBeenNthCalledWith(
                2,
                'performance',
        ...
        it('should short circuit if request throws a stale element exception', async () => {
            ...
            const opts = Object.assign(req.defaultOptions, {
                url: {
                    pathname:
                        '/session/foobar-123/element/some-sub-sub-elem-231/click',
                },
                body: { foo: 'bar' },
            })
            ...
            expect(vi.mocked(warn).mock.calls).toEqual([
        ...
        it('should not fail code due to an empty server response', async () => {
            ...
            await expect(req['_request'](opts)).rejects.toEqual(
        ...
        it('should retry requests but still fail', async () => {
            ...
            const opts = Object.assign(req.defaultOptions, {
                url: { pathname: '/failing' },
            })
            await expect(req['_request'](opts, undefined, 2)).rejects.toEqual(
        ...
        it('should retry and eventually respond', async () => {
            ...
            expect(await req['_request'](opts, undefined, 3)).toEqual({
                value: 'caught',
            })
        ...
        it('should fail if hub command is called on node', async () => {
            ...
            expect(result.message).toBe(
                'Command can only be called to a Selenium Hub',
            )
        })
        ...
        describe('"ETIMEDOUT" error', () => {
            ...
            it('should use error from "getTimeoutError" helper', async () => {
                ...
                const spy = vi
                    .spyOn(utils, 'getTimeoutError')
                    .mockReturnValue(timeoutErr)
                ...
                expect(req.emit).toHaveBeenNthCalledWith(2, 'response', {
                    error: timeoutErr,
                })
                expect(req.emit).toHaveBeenNthCalledWith(
                    3,
