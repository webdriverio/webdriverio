...
describe('utils', () => {
    it('isSuccessfulResponse', () => {
        expect(isSuccessfulResponse(200, { value: { some: 'result' } })).toBe(
            true,
        )
        expect(
            isSuccessfulResponse(404, {
                value: { error: new Error('foobar') },
            }),
        ).toBe(false)
        expect(
            isSuccessfulResponse(404, { value: { error: 'no such element' } }),
        ).toBe(true)
        expect(
            isSuccessfulResponse(404, {
                value: {
        ...
        expect(isSuccessfulResponse(undefined, { status: 7, value: {} })).toBe(
            false,
        )
        expect(isSuccessfulResponse(undefined, { status: 0, value: {} })).toBe(
            true,
        )
        expect(
            isSuccessfulResponse(undefined, {
                status: 7,
                value: { message: 'no such element: foobar' },
            }),
        ).toBe(true)
        expect(
            isSuccessfulResponse(200, {
                value: {
                    message: 'Unable to find element with xpath == //foobar',
                },
            }),
        ).toBe(true)
    })
    ...
    it('getPrototype', () => {
        ...
        expect(typeof mobilePrototype.getNetworkConnection.value).toBe(
            'function',
        )
        ...
        expect(typeof mobileChromePrototype.performActions.value).toBe(
            'function',
        )
        ...
        expect(typeof mobileChromePrototype.getNetworkConnection.value).toBe(
            'function',
        )
    ...
    it('getErrorFromResponseBody', () => {
        ...
        expect(getErrorFromResponseBody('expected', {})).toEqual(
            nonWebDriverError,
        )
        expect(
            getErrorFromResponseBody({ value: { message: 'expected' } }, {}),
        ).toEqual(expectedError)
        expect(
            getErrorFromResponseBody({ value: { class: 'expected' } }, {}),
        ).toEqual(expectedError)
    ...
    it('CustomRequestError', function () {
        ...
        error = new CustomRequestError(
            { value: { message: 'stale element reference' } },
            {},
        )
    ...
    describe('setupDirectConnect', () => {
        ...
        it('should do nothing if params contain no direct connect caps', function () {
            const client = new TestClient({ platformName: 'baz' }, {
                hostname: 'bar',
            } as any) as Client
        ...
        it('should do nothing if params contain incomplete direct connect caps', function () {
            const client = new TestClient(
                { platformName: 'baz', directConnectHost: 'baz' },
                { hostname: 'bar' } as any,
            ) as Client
            setupDirectConnect(client)
            expect(client.options.hostname).toEqual('bar')
    ...
    describe('getSessionError', () => {
        it('should return unchanged message', () => {
            expect(
                getSessionError({ message: 'foobar', name: 'Error' }, {}),
            ).toEqual('foobar')
        })

        it('should return "more info" if no message', () => {
            expect(getSessionError({ message: '', name: 'Error' }, {})).toEqual(
                'See wdio.* logs for more information.',
            )
        })
        ...
        it('should handle not properly set paths', () => {
            expect(
                getSessionError(
                    { message: 'unhandled request', name: 'Error' },
                    {},
                ),
        ...
        it('path: chromedriver, geckodriver, etc', () => {
            expect(
                getSessionError(new Error('HTTP method not allowed')),
        ...
        it('edge driver localhost issue', () => {
            expect(
                getSessionError(
                    new Error(
                        'Bad Request - Invalid Hostname 400 <br> HTTP Error 400',
                    ),
                ),
        ...
        it('wrong host port, port in use, illegal w3c cap passed to grid', () => {
            ...
            expect(message).toContain(
                'valid hostname:port or the port is not in use',
            )
            expect(message).toContain('add vendor prefix')
        })
        ...
        it('should hint for region issues for free-trial users', () => {
            const message = getSessionError(
                new Error(
                    'unknown error: failed serving request POST /wd/hub/session: Unauthorized',
                ),
                { hostname: 'https://ondemand.eu-central-1.saucelabs.com' },
            )
            expect(message).toContain(
                'Ensure this region is set in your configuration',
            )
        })
    })
    ...
    describe('startWebDriverSession', () => {
        it('attaches capabilities to the params object', async () => {
            ...
            expect(capabilities.browserName).toBe('mockBrowser')
        })
        ...
        it('should break if JSONWire and WebDriver caps are mixed together', async () => {
            ...
            const err: Error = await startWebDriverSession(params).catch(
                (err) => err,
            )
    ...
    describe('getTimeoutError', () => {
        const mkReqOpts = (
            opts: Options.RequestLibOptions = {},
        ...
        describe('should return error with', () => {
            it('command name as full endpoint', async () => {
                ...
                const reqOpts = mkReqOpts({
                    url: new URL('https://localhost:4445/wd/hub/session'),
                })
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(
                        'when running "https://localhost:4445/wd/hub/session"',
                    ),
                )
            })
            ...
            it('command name in shortened form', async () => {
                ...
                const reqOpts = mkReqOpts({
                    url: new URL(
                        'https://localhost:4445/wd/hub/session/abc123/url',
                    ),
                })
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching('when running "url"'),
                )
            })
            ...
            it('command method', async () => {
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(/when running .+ with method "GET"/),
                )
            })
            ...
            it('command args as stringified object', async () => {
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(
                        new RegExp(
                            `when running .+ with method .+ and args "${JSON.stringify(
                                cmdArgs,
                            )}"`,
                        ),
                    ),
            ...
            it('command args with base64 script', async () => {
                ;(
                    transformCommandLogResult as MockedFunction<any>
                ).mockReturnValueOnce('"<Script[base64]>"')
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(
                        /when running .+ with method .+ and args "<Script\[base64\]>"/,
                    ),
                )
            })
            ...
            it('command args with function script without extra wrapper', async () => {
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(
                        /when running .+ with method .+ and args "function\(\) {\nconsole\.log\("hi"\)\n}/,
                    ),
                )
            })
            ...
            it('command args with base64 screenshot', async () => {
                ;(
                    transformCommandLogResult as MockedFunction<any>
                ).mockReturnValueOnce('"<Screenshot[base64]>"')
                ...
                expect(timeoutErr.message).toEqual(
                    expect.stringMatching(
                        /when running .+ with method .+ and args "<Screenshot\[base64\]>"/,
                    ),
                )
            })
        })
