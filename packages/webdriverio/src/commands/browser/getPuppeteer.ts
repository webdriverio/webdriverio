...
export default async function getPuppeteer(this: WebdriverIO.Browser) {
    ...
    const caps =
        (this.capabilities as Capabilities.W3CCapabilities).alwaysMatch ||
        (this.capabilities as Capabilities.DesiredCapabilities)
    /**
    ...
    const requestedCapabilities =
        (this.requestedCapabilities as Capabilities.W3CCapabilities)
            ?.alwaysMatch ||
        (this.requestedCapabilities as Capabilities.DesiredCapabilities)
    const isAerokubeSession =
        requestedCapabilities['selenoid:options'] ||
        requestedCapabilities['moon:options']
    ...
    if (caps.browserName?.toLowerCase() === 'firefox') {
        ...
        const majorVersion = parseInt(
            caps.browserVersion.split('.').shift() || '',
            10,
        )
        if (majorVersion >= 79) {
            const reqCaps =
            ...
            const rdPort =
                ffOptions && ffOptions.debuggerAddress
                    ? ffOptions.debuggerAddress
                    : ffArgs?.[
                          ffArgs.findIndex(
                              (arg: string) => arg === FF_REMOTE_DEBUG_ARG,
                          ) + 1
                      ] ?? null

            if (!rdPort) {
                throw new Error(
                    "Could't find remote debug port in Firefox options",
                )
            }
