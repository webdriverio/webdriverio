 ...
 *
 */
export default async function reloadSession (this: Browser<'async'>) {
    const oldSessionId = (this as Browser<'async'>).sessionId
    ...
    if (Array.isArray(options.onReload) && options.onReload.length) {
        await Promise.all(options.onReload.map((hook) => hook(oldSessionId, (this as Browser<'async'>).sessionId)))
    }
