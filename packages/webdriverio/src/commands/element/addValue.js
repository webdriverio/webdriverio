...
export default function addValue (value, { translateToUnicode = true } = { }) {
    if (!this.isW3C) {
        return this.elementSendKeys(this.elementId, transformToCharString(value, { translateToUnicode: translateToUnicode }))
    }

    // Workaround https://github.com/appium/appium/issues/12085
    if (this.isMobile) {
        return this.elementSendKeys(this.elementId, transformToCharString(value, { translateToUnicode: translateToUnicode })
            .join(''), transformToCharString(value, { translateToUnicode: translateToUnicode }))
    }

    return this.elementSendKeys(this.elementId, transformToCharString(value, { translateToUnicode: translateToUnicode }).join(''))
}
