...
import type { ActionType } from '../../utils/actions/index.js'
import { KeyAction, PointerAction, WheelAction } from '../../utils/bidi/actions/index.js'
 ...
 * skip this by passing in `true` (e.g. `page.actions(...).perform(true)`).
 *
 * :::info
 *
 ...
 * <example>
    ...
    it('drag and drop using pointer action command', async () => {
        ...
        const origin = await page.$('#source')
        const targetOrigin = await page.$('#target')

        return page.action('pointer')
    ...
    it('should scroll using wheel action commands', async () => {
        console.log(await page.execute(() => window.scrollY)) // returns 0
        await page.action('wheel').scroll({
            deltaX: 0,
            deltaY: 500,
            duration: 200
        }).perform()
        console.log(await page.execute(() => window.scrollY)) // returns 500
    })
 * </example>
 *
 * @alias page.action
...
export function action (
    ...
    if (type === 'key') {
        return new KeyAction(this.browser, this.contextId)
    }
    if (type === 'pointer') {
        return new PointerAction(this.browser, this.contextId, opts)
    }
    if (type === 'wheel') {
        return new WheelAction(this.browser, this.contextId)
    }
