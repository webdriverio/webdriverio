...
export const elementErrorHandler = (fn: Function) => (commandName: string, commandFn: Function) => {
    return function elementErrorHandlerCallback (this: Element<'async'>, ...args: any[]) {
        return fn(commandName, async function elementErrorHandlerCallbackFn (this: Element<'async'>) {
...
) => (commandName: keyof Browser<'async'>) => {
    return wrapCommand(commandName, function (this: MultiRemoteBrowser<'async'>, ...args: any[]) {
        // @ts-ignore
