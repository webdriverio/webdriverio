...
export default class MultiRemote {
    ...
    modifier(wrapperClient: {
        ...
        this.baseInstance = new MultiRemoteDriver(
            this.instances,
            propertiesObject,
        )
    ...
    static elementWrapper(
        ...
        const prototype = {
            ...propertiesObject,
            ...clone(getPrototype('element')),
            scope: { value: 'element' },
        }

        const element = webdriverMonad(
            {},
            (client: WebdriverIO.MultiRemoteBrowser) => {
                /**
                 * attach instances to wrapper client
                 */
                for (const [i, identifier] of Object.entries(
     ...
     */
    commandWrapper(
        commandName: keyof (ProtocolCommands & BrowserCommandsType),
    ) {
        ...
        return wrapCommand(
            commandName,
            async function (
                this: MultiRemoteBrowser<'async'> | MultiRemoteElement,
                ...args: any[]
            ) {
                const mElem = this as MultiRemoteElement
                const scope = this.selector
                    ? Object.entries(
                          mElem.instances.reduce(
                              (ins, instanceName) => ({
                                  ...ins,
                                  [instanceName]: mElem[instanceName],
                              }),
                 ...
                 */
                if (commandName === '$') {
                    const elem = MultiRemote.elementWrapper(
                        instances,
                        result,
                    ...
                    return zippedResult.map((singleResult) =>
                        MultiRemote.elementWrapper(
                            instances,
                            singleResult,
                            this.__propertiesObject__,
                            self,
...
export class MultiRemoteDriver {
    ...
    on(
        ...
        this.instances.forEach((instanceName) =>
            this[instanceName].on(eventName, emitter),
        )
    ...
    once(
        ...
        this.instances.forEach((instanceName) =>
            this[instanceName].once(eventName, emitter),
        )
        return undefined as any
    }
    ...
    emit(
        ...
        return this.instances
            .map((instanceName) => this[instanceName].emit(eventName, emitter))
            .some(Boolean)
    }

    eventNames(this: WebdriverIO.MultiRemoteBrowser) {
        return this.instances.map((instanceName) =>
            this[instanceName].eventNames(),
        ) as any // special behavior of event methods for multiremote
    }

    getMaxListeners(this: WebdriverIO.MultiRemoteBrowser) {
        return this.instances.map((instanceName) =>
            this[instanceName].getMaxListeners(),
    ...
    listenerCount(this: WebdriverIO.MultiRemoteBrowser, eventName: string) {
        return this.instances.map((instanceName) =>
            this[instanceName].listenerCount(eventName),
        ) as any as number // special behavior of event methods for multiremote
    }

    listeners(this: WebdriverIO.MultiRemoteBrowser, eventName: string) {
        return this.instances
            .map((instanceName) => this[instanceName].listeners(eventName))
            .reduce((prev, cur) => {
                prev.concat(cur)
                return prev
            }, [])
    ...
    removeListener(
        ...
        this.instances.forEach((instanceName) =>
            this[instanceName].removeListener(eventName, emitter),
        )
        return undefined as any
    }
    ...
    removeAllListeners(
        ...
        this.instances.forEach((instanceName) =>
            this[instanceName].removeAllListeners(eventName),
        )
        return undefined as any
    }
}
