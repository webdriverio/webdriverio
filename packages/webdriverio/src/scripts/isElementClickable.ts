...
export default function isElementClickable(elem: HTMLElement) {
    ...
    function isOverlappingElementMatch(
        ...
        if (
            elementsFromPoint.some(function (elementFromPoint) {
                return (
                    elementFromPoint === elem ||
                    nodeContains(elem, elementFromPoint)
                )
            })
        ) {
            return true
        ...
        elemsWithShadowRoot = elemsWithShadowRoot.filter(function (
            x: HTMLElement,
        ) {
