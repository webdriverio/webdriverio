...
import type { ElementArray, Selector } from '../types'
import {
    getBrowserObject,
    getElementFromResponse,
    getPrototype as getWDIOPrototype,
} from './index.js'
...
export const getElement = function findElement(
    ...
    const propertiesObject = {
         ...
         */
        ...Object.entries(clone(browser.__propertiesObject__)).reduce(
    ...
    const elementInstance = element(
        ...
        elementErrorHandler(wrapCommand),
    )
...
export const getElements = function getElements(
    ...
    const elements = elemResponse.map(
        (res: ElementReference | WebdriverIO.Element | Error, i) => {
            /**
            ...
            const element = webdriverMonad(
                this.options,
                (client: WebdriverIO.Element) => {
                    const elementId = getElementFromResponse(
                        res as ElementReference,
                    )
            ...
            const elementInstance = element(
                (this as WebdriverIO.Browser).sessionId,
                elementErrorHandler(wrapCommand),
            )

            const origAddCommand =
                elementInstance.addCommand.bind(elementInstance)
            elementInstance.addCommand = (name: string, fn: Function) => {
                browser.__propertiesObject__[name] = { value: fn }
                origAddCommand(name, fn)
            }
