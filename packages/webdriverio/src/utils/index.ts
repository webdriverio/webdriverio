    ...
    if (parsedValue.value?.indexOf('rgb') === 0) {
        ...
        try {
            ...
            if (
                ...
                parsedValue.parsed.type === 'number' &&
                parsedValue.parsed.unit === ''
            ) {
...
function fetchElementByJSFunction(
    ...
    const script = function (elem: HTMLElement, id: string) {
        ...
        .toString()
        .replace('selector', `(${selector.toString()})`)
    return getBrowserObject(scope).execute(
...
function isElement(o: Selector) {
    return typeof HTMLElement === 'object'
        ...
        : o &&
              ...
              typeof (o as HTMLElement).nodeName === 'string'
}
...
export async function findElement(
    ...
    if (
        ...
        selector.startsWith(DEEP_SELECTOR)
    ) {
        const notFoundError = new Error(
            `shadow selector "${selector.slice(
                DEEP_SELECTOR.length,
            )}" did not return an HTMLElement`,
        )
        let elem: ElementReference | ElementReference[] =
            await browserObject.execute(
                ...
                ((this as WebdriverIO.Element).elementId
                    ? this
                    : undefined) as any as Element | Document,
            )
    ...
    if (
        ...
        typeof (selector as CustomStrategyReference).strategy === 'function'
    ) {
        const { strategy, strategyName, strategyArguments } =
            selector as CustomStrategyReference
        const notFoundError = new Error(
            `Custom Strategy "${strategyName}" did not return an HTMLElement`,
        )
    ...
    if (typeof selector === 'string' || isPlainObject(selector)) {
        const { using, value } = findStrategy(
            selector as string,
            this.isW3C,
            this.isMobile,
        )
    ...
    if (typeof selector === 'function') {
        const notFoundError = new Error(
            `Function selector "${selector.toString()}" did not return an HTMLElement`,
        )
    ...
    if (isElement(selector)) {
        ...
        selector = ((id: string) =>
            window.__wdio_element[id]) as any as ElementFunction
...
export async function findElements(
    ...
    if (
        ...
        const elems: ElementReference | ElementReference[] =
            await browserObject.execute(
                `return (${querySelectorAllDeep}).apply(null, arguments)`,
                true,
                selector.slice(DEEP_SELECTOR.length),
    ...
    if (
        ...
        const { strategy, strategyArguments } =
            selector as CustomStrategyReference
        const elems = await browserObject.execute(
            strategy,
            ...strategyArguments,
        )
        const elemArray = Array.isArray(elems)
            ? (elems as ElementReference[])
            : [elems]
        return elemArray.filter((elem) => elem && getElementFromResponse(elem))
    }
...
export function verifyArgsAndStripIfElement(args: any) {
    function verify(arg: any) {
        if (
            arg &&
            typeof arg === 'object' &&
            arg.constructor.name === 'Element'
        ) {
            const elem = arg as WebdriverIO.Element
            if (!elem.elementId) {
                throw new Error(
                    `The element with selector "${elem.selector}" you are trying to pass into the execute method wasn't found`,
                )
            }
...
export async function getElementRect(scope: WebdriverIO.Element) {
    ...
    if (
        Object.keys(defaults).some(
            (key: keyof typeof defaults) => rect[key] == null,
        )
    ) {
        /* istanbul ignore next */
        const rectJs = await getBrowserObject(scope).execute(function (
            this: Window,
            el: HTMLElement,
        ) {
        ...
        Object.keys(defaults).forEach((key: keyof typeof defaults) => {
            ...
            if (rectJs && typeof rectJs[key] === 'number') {
                ...
                throw new Error(
                    'Failed to receive element rects via execute command',
                )
            }
        })
    }
...
export async function assertDirectoryExists(filepath: string) {
    const exist = await fs.access(path.dirname(filepath)).then(
        () => true,
        () => false,
    )
    if (!exist) {
...
export function getScrollPosition(scope: WebdriverIO.Element) {
    return getBrowserObject(scope).execute(
        /* istanbul ignore next */ function (this: Window) {
...
export async function hasElementId(element: WebdriverIO.Element) {
    ...
    if (!element.elementId) {
        ...
        element.elementId = (
            await command(element.selector as string)
        ).elementId
    }
...
export function addLocatorStrategyHandler(
    scope: WebdriverIO.Browser | WebdriverIO.MultiRemoteBrowser,
) {
    return (
        name: string,
        func: (
 ...
 */
export const isStub = (automationProtocol?: string) =>
    automationProtocol === './protocol-stub.js'

export const getAutomationProtocol = async (
    ...
    if (
        !SUPPORTED_BROWSER.includes(caps.browserName?.toLowerCase() as string)
    ) {
        return 'webdriver'
    }
     ...
     */
    const resp: http.IncomingMessage | { error: Error } = await new Promise(
        (resolve) => {
    ...
    if (
        (driverEndpointHeaders as any).req &&
        (driverEndpointHeaders as any).req.agent
    ) {
