...
export default class DevtoolsInterception extends Interception {
    static handleRequestInterception(
        client: CDPSession,
        mocks: Set<Interception>,
    ): (event: Event) => Promise<void | ClientResponse> {
        return async (event) => {
            ...
            const responseHeaders = eventResponseHeaders.reduce(
                (headers, { name, value }) => {
                    headers[name] = value
                    return headers
                },
            ...
            for (const mock of mocks) {
                ...
                if (
                    ...
                    (mock.respondOverwrites.length === 0 || // nothing to do in Request stage
                        (!mock.respondOverwrites[0].errorReason && // skip if not going to abort a request
                            ...
                            mock.respondOverwrites[0].params.fetchResponse !==
                                false))
                ) {
                    continue
                }
                ...
                if (
                    ...
                    filterHeaders(
                        request.headers,
                        mock.filterOptions.requestHeaders,
                    ) ||
                    filterHeaders(
                        responseHeaders,
                        mock.filterOptions.headers,
                    ) ||
                    filterRequest(
                        request.postData,
                        mock.filterOptions.postData,
                    ) ||
                    filterStatusCode(
                        responseStatusCode,
                        mock.filterOptions.statusCode,
                    )
                ) {
                    continue
                }

                const { body, base64Encoded = undefined } = isRequest
                    ? { body: '' }
                    : await client
                          .send('Fetch.getResponseBody', { requestId })
                          .catch(/* istanbul ignore next */ () => ({} as any))

                request.body = base64Encoded
                ...
                const {
                    ...
                    overwrite,
                    params = {},
                } = mock.respondOverwrites[0].sticky
                ...
                if (overwrite !== undefined) {
                    ...
                    let responseCode =
                        typeof params.statusCode === 'function'
                            ? params.statusCode(request)
                            : params.statusCode || responseStatusCode
                    let responseHeaders: HeaderEntry[] = [
                        ...eventResponseHeaders,
                        ...Object.entries(
                            typeof params.headers === 'function'
                     ...
                     */
                    const responseFilePath = path.isAbsolute(newBody)
                        ? newBody
                        : path.join(process.cwd(), newBody)
                    const responseFileAccessible = await fs
                        .access(responseFilePath)
                        .then(
                            () => true,
                            () => false,
                        )
                    ...
                    return client
                        .send('Fetch.fulfillRequest', {
                            ...
                            body: isBodyUndefined
                                ? undefined
                                : (newBody instanceof Buffer
                                      ? newBody
                                      : Buffer.from(newBody as string, 'utf8')
                                  ).toString('base64'),
                        })
    ...
    abort(errorReason: Protocol.Network.ErrorReason, sticky: boolean = true) {
        if (
            ...
            !ERROR_REASON.includes(errorReason)
        ) {
            throw new Error(
                `Invalid value for errorReason, allowed are: ${ERROR_REASON.join(
                    ', ',
                )}`,
            )
        }
...
const filterHeaders = (
    ...
    expected?: ExpectParameter<Record<string, string>>,
) => {
    if (typeof expected === 'undefined') {
        return false
    }
...
const filterRequest = (
    postData?: string,
    expected?: ExpectParameter<string | undefined>,
) => {
    if (typeof expected === 'undefined') {
        return false
    }
...
const filterStatusCode = (
    statusCode: number,
    expected?: ExpectParameter<number>,
) => {
    if (typeof expected === 'undefined') {
        return false
    }
