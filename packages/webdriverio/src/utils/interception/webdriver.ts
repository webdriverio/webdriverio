...
export default class WebDriverInterception extends Interception {
    ...
    async init() {
        ...
        const { mockId } = await this.browser.mockRequest(
            this.url,
            this.filterOptions,
        )
        this.mockId = mockId
    ...
    clear() {
        return this.browser.call(async () =>
            this.browser.clearMockCalls(this.mockId as string),
        )
    }
    ...
    restore() {
        return this.browser.call(async () =>
            this.browser.clearMockCalls(this.mockId as string, true),
        )
    }
    ...
    respond(overwrite: MockOverwrite, params: MockResponseParams = {}) {
        return this.browser.call(async () =>
            this.browser.respondMock(this.mockId as string, {
                overwrite,
                params,
                sticky: true,
            }),
