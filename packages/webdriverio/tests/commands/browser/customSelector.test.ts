...
describe('custom$', () => {
    ...
    it('should fetch element', async () => {
        ...
        expect(typeof (elem.selector as CustomStrategyReference).strategy).toBe(
            'function',
        )
        expect((elem.selector as CustomStrategyReference).strategyName).toBe(
            'test',
        )
        expect(
            (elem.selector as CustomStrategyReference).strategyArguments,
        ).toHaveLength(1)
        expect(
            (elem.selector as CustomStrategyReference).strategyArguments[0],
        ).toBe('.test')
    })
    ...
    it('should throw error if no element is returned from the user script', async () => {
        ...
        const err = (await browser
            .custom$('test-no-element', '.foo')
            // @ts-ignore use of sync types
            .catch((err: Error) => err)) as Error

        expect(err.message).toBe(
            'Your locator strategy script must return an element',
        )
    })
})
