...
describe('isEnabled test', () => {
    it('should allow to check if an element is enabled', async () => {
        ...
        expect((vi.mocked(got).mock.calls[1][0] as any).pathname).toBe(
            '/session/foobar-123/execute/sync',
        )
        expect((vi.mocked(got).mock.calls[1][1] as any).json.script).toBe(
            'return ((a, b, c) => a + b + c).apply(null, arguments)',
        )
        expect((vi.mocked(got).mock.calls[1][1] as any).json.args).toEqual([
            1, 2, 3,
        ])
    })
