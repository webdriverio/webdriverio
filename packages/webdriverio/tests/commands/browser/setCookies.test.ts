...
describe('setCookies', () => {
    ...
    it('should output the expected format', async () => {
        ...
        expect(got.mock.calls[1][1].json).toEqual({
            cookie: { name: 'cookie1', value: 'dummy-value-1' },
        })
    })
    ...
    it('can be called multiple times', async () => {
        ...
        expect(got.mock.calls[1][0].pathname).toBe('/session/foobar-123/cookie')
        expect(got.mock.calls[1][1].json).toEqual({
            cookie: { name: 'cookie2', value: 'dummy-value-1' },
        })
    })
    ...
    it('should work with multiple objects passed', async () => {
        ...
        cookies.forEach((cookie, i) => {
            ...
            expect(got.mock.calls[i][0].pathname).toBe(
                '/session/foobar-123/cookie',
            )
    ...
    it('should throw error if invalid arguments are passed', async () => {
        ...
        await expect(browser.setCookies([2])).rejects.toEqual(
            new Error(
