...
describe('setTimeout', () => {
    it('should set timeout', async () => {
        ...
        expect(got.mock.calls[4][0].pathname).toBe(
            '/session/foobar-123/timeouts',
        )
        ...
        expect(got.mock.calls[5][0].pathname).toBe(
            '/session/foobar-123/timeouts',
        )
    ...
    it('should set timeout (no w3c)', async () => {
        ...
        expect(got.mock.calls[2][1].json).toEqual({
            type: 'page load',
            ms: 10000,
        })
        ...
        expect(got.mock.calls[6][0].pathname).toBe(
            '/session/foobar-123/timeouts',
        )
    ...
    it('should throw error on setting invalid timeout', async () => {
        ...
        const invalidTimeoutParameterError = new Error(
            'Parameter for "setTimeout" command needs to be an object',
        )
        // @ts-expect-error invalid param
        await expect(browser.setTimeout('0')).rejects.toEqual(
            invalidTimeoutParameterError,
        )
        // @ts-expect-error invalid param
        await expect(browser.setTimeout(5000)).rejects.toEqual(
            invalidTimeoutParameterError,
        )

        const invalidTimeoutValueError = new Error(
            'Specified timeout values are not valid integer (see https://webdriver.io/docs/api/browser/setTimeout for documentation).',
        )
        await expect(
            browser.setTimeout({ implicit: Number.MAX_SAFE_INTEGER + 1 }),
        ).rejects.toEqual(invalidTimeoutValueError)
        await expect(browser.setTimeout({ pageLoad: -2000 })).rejects.toEqual(
            invalidTimeoutValueError,
        )
        // @ts-expect-error invalid param
        await expect(browser.setTimeout({ 'page load': null })).rejects.toEqual(
            invalidTimeoutValueError,
        )
        // @ts-expect-error invalid param
        await expect(browser.setTimeout({ script: '4000' })).rejects.toEqual(
            invalidTimeoutValueError,
        )
        // @ts-expect-error invalid param
        await expect(
            browser.setTimeout({ timeouts: { script: 5000 } }),
        ).rejects.toEqual(invalidTimeoutValueError)
    })
