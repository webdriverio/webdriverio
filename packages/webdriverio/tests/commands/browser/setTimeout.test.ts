...
describe('setTimeout', () => {
    ...
    it('should set timeout (no w3c)', async () => {
        ...
        expect((vi.mocked(fetch).mock.calls[1][0] as any).pathname)
            .toBe('/session/foobar-123/timeouts')
        expect((vi.mocked(fetch).mock.calls[1][1] as any).json)
        ...
        expect((vi.mocked(fetch).mock.calls[2][0] as any).pathname)
            .toBe('/session/foobar-123/timeouts')
        expect((vi.mocked(fetch).mock.calls[2][1] as any).json)
        ...
        expect((vi.mocked(fetch).mock.calls[4][1] as any).method).toBe('POST')
        expect((vi.mocked(fetch).mock.calls[4][0] as any).pathname)
            .toBe('/session/foobar-123/timeouts')
        expect((vi.mocked(fetch).mock.calls[4][1] as any).json)
            .toEqual({ type: 'implicit', ms: 0 })
        expect((vi.mocked(fetch).mock.calls[5][1] as any).method).toBe('POST')
        expect((vi.mocked(fetch).mock.calls[5][0] as any).pathname)
            .toBe('/session/foobar-123/timeouts')
        expect((vi.mocked(fetch).mock.calls[5][1] as any).json)
            .toEqual({ type: 'page load', ms: 300000 })
        expect((vi.mocked(fetch).mock.calls[6][1] as any).method).toBe('POST')
