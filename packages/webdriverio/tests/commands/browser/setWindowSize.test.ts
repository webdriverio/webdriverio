...
describe('setWindowSize', () => {
    ...
    it('should resize W3C browser window', async () => {
        ...
        expect(got.mock.calls[1][0].pathname).toBe(
            '/session/foobar-123/window/rect',
        )
    ...
    it('should resize NO-W3C browser window', async () => {
        ...
        expect(got.mock.calls[1][1].method).toBe('POST')
        expect(got.mock.calls[1][0].pathname).toBe(
            '/session/foobar-123/window/current/size',
        )
    ...
    describe('input checks', () => {
        it('should throw error if width or height is not number', async () => {
            const invalidTypeError =
                'setWindowSize expects width and height of type number'

            // @ts-ignore tets invalid parameter
            let err = await browser
                .setWindowSize('777', 888)
                .catch((err: Error) => err)
        ...
        it('should throw error if width or height not in the 0 to 2^31 − 1 range', async () => {
            const invalidValueError =
                'setWindowSize expects width and height to be a number in the 0 to 2^31 − 1 range'

            // @ts-ignore test invalid parameter
            let err: Error = await browser
                .setWindowSize(-1, 500)
                .catch((err: Error) => err)
            expect(err.message).toBe(invalidValueError)
            // @ts-ignore test invalid parameter
            err = await browser
                .setWindowSize(Number.MAX_SAFE_INTEGER + 100, 500)
                .catch((err: Error) => err)
            expect(err.message).toBe(invalidValueError)
            // @ts-ignore test invalid parameter
            err = await browser
                .setWindowSize(-0.01, 500)
                .catch((err: Error) => err)
            expect(err.message).toBe(invalidValueError)
        })
    })
