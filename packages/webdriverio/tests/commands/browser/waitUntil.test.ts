...
describe('waitUntil', () => {
    ...
    it.each([false, '', 0])(
        'Should throw an error when the waitUntil times out e.g. doesnt resolve to a truthy value: %i',
        async () => {
            let error
            let val
            // @ts-ignore uses expect-webdriverio
    ...
    it('Should throw an error when the promise is rejected', async () => {
        ...
        try {
            val = await browser.waitUntil(
                () =>
                    new Promise<boolean>((resolve, reject) =>
                        setTimeout(() => reject(new Error('foobar')), 200),
                    ),
                {
                    timeout: 500,
                    timeoutMsg: 'Timed Out',
            ...
            expect(error.message).toContain(
                'waitUntil condition failed with the following reason: foobar',
            )
            expect(val).toBeUndefined()
        }
    })
    ...
    it('Should throw an error when the promise is rejected without error message', async () => {
        ...
        try {
            ...
            expect(err.message).toContain(
                'waitUntil condition failed with the following reason: Error',
            )
            expect(val).toBeUndefined()
        }
    })
    ...
    it('Should use default timeout setting from config if passed in value is not a number', async () => {
        ...
        try {
            ...
            expect(error.message).toMatch(
                /waitUntil condition timed out after \d+ms/,
            )
            expect(val).toBeUndefined()
        }
    })
    ...
    it.each([true, 'false', 123])(
        'Should pass for a truthy resolved value: %i',
        async (n) => {
            let error
            let val
            // @ts-ignore uses expect-webdriverio
