...
describe('click test', () => {
    ...
    it('should allow to left click on an element (no w3c)', async () => {
        ...
        expect(vi.mocked(fetch).mock.calls[3][1]?.body)
            .toStrictEqual(JSON.stringify({ element: 'some-elem-123', xoffset: 25, yoffset: 15 }))
        // @ts-expect-error mock implementation
        expect(vi.mocked(fetch).mock.calls[4][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[4][1]?.body).toStrictEqual(JSON.stringify({ button: 0 }))

        await elem.click({ button: 0 })
        // @ts-expect-error mock implementation
        expect(vi.mocked(fetch).mock.calls[6][0].pathname)
            .toBe('/session/foobar-123/moveto')
        expect(vi.mocked(fetch).mock.calls[6][1]?.body)
            .toStrictEqual(JSON.stringify({ element: 'some-elem-123', xoffset: 25, yoffset: 15 }))
        // @ts-expect-error mock implementation
        expect(vi.mocked(fetch).mock.calls[7][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[7][1]?.body).toStrictEqual(JSON.stringify({ button: 0 }))
    })
    ...
    it('should allow to right click on an element (no w3c)', async () => {
        ...
        expect(vi.mocked(fetch).mock.calls[4][1]?.body)
            .toStrictEqual(JSON.stringify({ button: 2 }))
        ...
        expect(vi.mocked(fetch).mock.calls[7][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[7][1]?.body)
            .toStrictEqual(JSON.stringify({ button: 2 }))
    })
    ...
    it('should allow to middle click on an element (no w3c)', async () => {
        ...
        expect(vi.mocked(fetch).mock.calls[4][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[4][1]?.body)
            .toStrictEqual(JSON.stringify({ button: 1 }))
    ...
    it('should allow to left click on an element with an offset without passing a button type (no w3c)', async () => {
        ...
        expect(vi.mocked(fetch).mock.calls[3][0].pathname)
            .toBe('/session/foobar-123/moveto')
        expect(vi.mocked(fetch).mock.calls[3][1]?.body)
            .toStrictEqual(JSON.stringify({ element: 'some-elem-123', xoffset: 25, yoffset: 45 }))
        // @ts-expect-error mock implementation
        expect(vi.mocked(fetch).mock.calls[4][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[4][1]?.body)
            .toStrictEqual(JSON.stringify({ button: 0 }))
    })
    ...
    it('should allow to right click on an element with an offset and passing a button type (no w3c)', async () => {
        ...
        expect(vi.mocked(fetch).mock.calls[3][0].pathname)
            .toBe('/session/foobar-123/moveto')
        expect(vi.mocked(fetch).mock.calls[3][1]?.body)
            .toStrictEqual(JSON.stringify({ element: 'some-elem-123', xoffset: 65, yoffset: 45 }))
        // @ts-expect-error mock implementation
        expect(vi.mocked(fetch).mock.calls[4][0].pathname)
            .toBe('/session/foobar-123/click')
        expect(vi.mocked(fetch).mock.calls[4][1]?.body).toStrictEqual(JSON.stringify({ button: 2 }))
    })
