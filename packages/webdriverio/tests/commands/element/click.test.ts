...
describe('click test', () => {
    it('should allow to left click on an element without passing a button type', async () => {
        ...
        expect(got.mock.calls[2][0].pathname).toBe(
            '/session/foobar-123/element/some-elem-123/click',
        )
    })
    ...
    it('should allow to right click on an element with an offset and passing a button type', async () => {
        ...
        expect(got.mock.calls[2][1].json.actions[0].actions[0].type).toBe(
            'pointerMove',
        )
        expect(got.mock.calls[2][1].json.actions[0].actions[0].x).toBe(40)
        expect(got.mock.calls[2][1].json.actions[0].actions[0].y).toBe(30)
        expect(
            got.mock.calls[2][1].json.actions[0].actions[1],
        ).toMatchSnapshot()
        expect(got.mock.calls[2][1].json.actions[0].actions[2]).toStrictEqual({
            type: 'pointerUp',
            button: 2,
        })
    })
    ...
    it('should allow to left click on an element (no w3c)', async () => {
        ...
        expect(got.mock.calls[3][0].pathname).toBe('/session/foobar-123/moveto')
        expect(got.mock.calls[3][1].json).toStrictEqual({
            element: 'some-elem-123',
            xoffset: 25,
            yoffset: 15,
        })
        expect(got.mock.calls[4][0].pathname).toBe('/session/foobar-123/click')
        expect(got.mock.calls[4][1].json).toStrictEqual({ button: 0 })

        await elem.click({ button: 0 })

        expect(got.mock.calls[6][0].pathname).toBe('/session/foobar-123/moveto')
        expect(got.mock.calls[6][1].json).toStrictEqual({
            element: 'some-elem-123',
            xoffset: 25,
            yoffset: 15,
        })
        expect(got.mock.calls[7][0].pathname).toBe('/session/foobar-123/click')
    ...
    it('should allow to right click on an element (no w3c)', async () => {
        ...
        expect(got.mock.calls[7][0].pathname).toBe('/session/foobar-123/click')
        expect(got.mock.calls[7][1].json).toStrictEqual({ button: 2 })
    })
    ...
    it('should allow to middle click on an element (no w3c)', async () => {
        ...
        expect(got.mock.calls[7][0].pathname).toBe('/session/foobar-123/click')
        expect(got.mock.calls[7][1].json).toStrictEqual({ button: 1 })
    })
    ...
    it('should throw an error if no valid coördicates are passed', async () => {
        ...
        // @ts-expect-error invalid param
        expect(elem.click({ x: 'not-suppported' })).rejects.toThrow(
    ...
    it('should throw an error if no valid button type is passed', async () => {
        ...
        expect(elem.click({ button: 'not-suppported' })).rejects.toThrow(
            'Button type not supported.',
        )
    })
    ...
    it('should not call releaseAction when skipRelease is true', async () => {
        ...
        expect(got.mock.calls.length).toBe(4)
        expect(got.mock.calls[3][0].pathname).toBe(
            '/session/foobar-123/actions',
        )
        expect(got.mock.calls[3][1].method).toBe('DELETE')
