...
describe('custom$', () => {
    ...
    it('should fetch element', async () => {
        browser.addLocatorStrategy(
            'test',
            (selector: string, root: HTMLElement) =>
                [
                    {
        ...
        expect(
            typeof (elems[0].selector as CustomStrategyReference).strategy,
        ).toBe('function')
        expect(
            (elems[0].selector as CustomStrategyReference).strategyName,
        ).toBe('test')
        expect(
            (elems[0].selector as CustomStrategyReference).strategyArguments,
        ).toHaveLength(2)
        expect(
            (elems[0].selector as CustomStrategyReference).strategyArguments[0],
        ).toBe('.test')
        expect(
            (elems[0].selector as CustomStrategyReference).strategyArguments[1],
        ).toBe(elem)
