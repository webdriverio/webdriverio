...
describe('isDisplayed test', () => {
    ...
    it('should refetch element if non existing', async () => {
        ...
        expect(request.mock.calls[0][0].uri.path).toBe('/session/foobar-123/element')
        expect(request.mock.calls[1][0].uri.path).toBe('/session/foobar-123/element/some-elem-123/displayed')
    })
    ...
    describe('isElementDisplayed script', () => {
        it('should be used if safari and w3c', async () => {
            ...
            expect(await elem.isDisplayed()).toBe(true)
            expect(request).toBeCalledTimes(1)
            expect(request.mock.calls[0][0].uri.path).toBe('/session/foobar-123/execute/sync')
