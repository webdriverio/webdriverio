...
describe('moveTo', () => {
    it('should do a moveTo without params', async () => {
        ...
        expect(
            got.mock.calls[4][1].json.actions[0].actions[0],
        ).toMatchSnapshot()
    })
    ...
    it('should do a moveTo with params if getElementRect returned empty object', async () => {
        ...
        got.setMockResponse([
            {},
            { x: 5, y: 10, height: 33, width: 44 },
            { scrollX: 0, scrollY: 0 },
        ])
        await elem.moveTo({ xOffset: 5, yOffset: 10 })
        expect(
            got.mock.calls[5][1].json.actions[0].actions[0],
        ).toMatchSnapshot()
    })
    ...
    it('should do a moveTo with params if getElementRect and getBoundingClientRect returned empty object', async () => {
        ...
        // @ts-ignore mock feature
        got.setMockResponse([{}, {}])
        await expect(elem.moveTo({ xOffset: 5, yOffset: 10 })).rejects.toThrow(
    ...
    it('should do a moveTo without params (no-w3c)', async () => {
        ...
        expect(got.mock.calls[3][1].json).toEqual({
    ...
    it('should return integer values when provided float getScrollPosition params', async () => {
        ...
        got.setMockResponse([
            {},
            { x: 4, y: 9, height: 35, width: 42 },
            { scrollX: 2.1, scrollY: 3.3 },
        ])
        await elem.moveTo({ xOffset: 5, yOffset: 10 })
        expect(
            got.mock.calls[5][1].json.actions[0].actions[0],
