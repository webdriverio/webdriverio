...
describe('selectByIndex test', () => {
    ...
    it('should select by index', async () => {
        ...
        expect(got.mock.calls[2][0].pathname).toBe(
            '/session/foobar-123/element/some-elem-123/elements',
        )
        expect(got.mock.calls[3][0].pathname).toBe(
            '/session/foobar-123/element/some-elem-456/click',
        )
    ...
    it('should throw an error when index < 0', async () => {
        ...
        try {
            ...
            expect(err.toString()).toBe(
                'Error: Index needs to be 0 or any other positive number',
            )
        }
    })
    ...
    it('should throw if there are no options elements', async () => {
        ...
        try {
            ...
            expect(err.toString()).toBe(
                "Error: Select element doesn't contain any option element",
            )
        }
    })
    ...
    it('should throw if index is out of range', async () => {
        ...
        const mockElem = {
            ...
            findElementsFromElement: vi
                .fn()
                .mockReturnValue(Promise.resolve([{ elem: 1 }])),
        }
        // @ts-ignore mock feature
        mockElem.selectByIndex = elem.selectByIndex.bind(mockElem)
        ...
        try {
            ...
            expect(err.toString()).toBe(
                'Error: Can\'t call selectByIndex on element with selector "foobar" because element wasn\'t found',
            )
        }
    })
    ...
    it('should throw if index is out of rangew', async () => {
        ...
        try {
            ...
            expect(err.toString()).toBe(
                'Error: Option with index "3" not found. Select element only contains 3 option elements',
            )
        }
    })
})
