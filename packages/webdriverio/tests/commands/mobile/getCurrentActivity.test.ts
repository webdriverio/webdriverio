...
describe('getCurrentActivity', () => {
    ...
    describe('legacy driver fallback (mobile: getCurrentActivity returns unknown method)', () => {
        beforeEach(async () => {
            browser = await remote({
                baseUrl: 'http://foobar.com',
                capabilities: {
        ...
        it('should fall back to appiumGetCurrentActivity and log a warning', async () => {
            vi.spyOn(browser, 'execute').mockRejectedValue(new Error('unknown method: mobile: getCurrentActivity'))
            const appiumGetCurrentActivitySpy = vi.spyOn(browser, 'appiumGetCurrentActivity').mockResolvedValue('.MainActivity')

            const result = await browser.getCurrentActivity()

            expect(appiumGetCurrentActivitySpy).toHaveBeenCalledWith()
            expect(result).toBe('.MainActivity')
            expect(log.warn).toHaveBeenCalledWith(expect.stringContaining('mobile: getCurrentActivity'))
        })

        it('should fall back to appiumGetCurrentActivity on unknown command', async () => {
            vi.spyOn(browser, 'execute').mockRejectedValue(new Error('unknown command'))
            const appiumGetCurrentActivitySpy = vi.spyOn(browser, 'appiumGetCurrentActivity').mockResolvedValue('.SplashActivity')

            const result = await browser.getCurrentActivity()

            expect(appiumGetCurrentActivitySpy).toHaveBeenCalledWith()
            expect(result).toBe('.SplashActivity')
        })
    })
})
