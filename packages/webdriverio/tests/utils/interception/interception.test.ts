...
test('allows custom error message', async () => {
    ...
    expect(err.message).toBe(
        'waitForResponse failed with the following reason: bug',
    )
})

test('isMatchingRequest', () => {
    expect(
        NetworkInterception.isMatchingRequest(
            '**/foo.*.js',
            'http://json.org/foo.bar.js',
        ),
    ).toBe(true)
    expect(
        NetworkInterception.isMatchingRequest(
            '**/foo.*.js',
            'http://json.org/foo.js',
        ),
    ).toBe(false)
    expect(
        NetworkInterception.isMatchingRequest(
