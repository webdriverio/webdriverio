...
const latestRelease = shell.exec(
    'git describe --tags `git rev-list --tags --max-count=100`',
    { silent: true }
).stdout.split('\n').filter((v) => v && !v.startsWith('v6') && !v.includes('-')).shift()

console.log(`Current version: ${latestRelease}`)
