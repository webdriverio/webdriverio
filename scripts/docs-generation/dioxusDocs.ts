...
const DOCS_SHA = '34f791f7984e1a32e816251f318ee64e48c23cf6'
const DOCS_SOURCE_DIR = 'packages/dioxus-service/docs'
const WEBSITE_DOCS_PATH = ['website', 'docs', 'desktop-testing', 'dioxus']
const PUBLISHED_URL_PREFIX = '/docs/desktop-testing/dioxus'

export async function generateDioxusDocs () {
    const basePath = path.join(__dirname, '..', '..')
    const dioxusDocsPath = path.join(basePath, ...WEBSITE_DOCS_PATH)
    await fs.mkdir(dioxusDocsPath, { recursive: true })

    const rewriteLinks = buildLinkRewriter(allDocs, PUBLISHED_URL_PREFIX)

    for (const [id, { sourcePath, title }] of Object.entries(allDocs)) {
        const newDocsPath = path.join(dioxusDocsPath, `${id}.md`)
