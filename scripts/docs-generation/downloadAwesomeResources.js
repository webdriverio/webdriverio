...
export async function downloadAwesomeResources() {
    ...
    const content = await downloadFromGitHub(
        'https://raw.githubusercontent.com/webdriverio-community/awesome-webdriverio',
        'main',
    )
    let lines = content.toString().split(/\n/)
    lines = lines.slice(
        lines.findIndex((line) => line.includes('WebdriverIO Ecosystem')),
    )
