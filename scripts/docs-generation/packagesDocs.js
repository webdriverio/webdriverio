...
import { buildPreface, getSubPackages } from '../utils/helpers.js'
...
export function generateReportersAndServicesDocs(sidebars) {
    ...
    for (const [type, [namePlural, nameSingular]] of Object.entries(plugins)) {
        const pkgs = packages.filter(
            (pkg) => pkg.endsWith(`-${type}`) && pkg.split('-').length > 2,
        )
        for (const pkg of pkgs) {
            ...
            const preface = buildPreface(
                ...
                pkgName,
                nameSingular,
                `https://github.com/webdriverio/webdriverio/edit/main/packages/${pkg}/README.md`,
            )
