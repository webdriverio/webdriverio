...
const mainPackageFolderFiles = [
  ...
  "publishConfig": {
    ...
    {
        name: 'README.md',
        content: `WebdriverIO ${packageName.toUpperCase()} ${packageType
            .charAt(0)
            .toUpperCase()}${packageType.substr(1)}\n========================`,
    },
]
...
if (existingPackages.includes(mainPackageFolder)) {
    throw new Error(
        `Package '${fullPackageName}' already exists. Please choose another name.`,
    )
}

// create package structure
;[mainPackageFolder, srcPackageFolder, testsPackageFolder].map((folder) =>
    shell.mkdir(folder),
)

// create main folder files
mainPackageFolderFiles.map((file) =>
    createFile(path.join(mainPackageFolder, file.name), file.content),
)

// create src folder files
srcPackageFolderFiles.map((file) =>
    createFile(path.join(srcPackageFolder, file.name), file.content),
)

// create tests folder files
testsPackageFolderFiles.map((file) =>
    createFile(path.join(testsPackageFolder, file.name), file.content),
)
