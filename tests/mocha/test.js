...
describe('Mocha smoke test', () => {
    ...
    it.skip('should allow to use async chaining within call command', () => {
        browser.isExistingScenario()
        browser.call(async () => {
            const el = await browser.$('body')
            assert.equal(await el.$('.selector-1').isExisting(), true)
    ...
    it('should allow to use async chaining within waitUntil command', () => {
        browser.isExistingScenario()
        browser.waitUntil(async () => {
            const el = await browser.$('body')
            const result = await el.$('.selector-1').isExisting()
            return result
        })
    })
