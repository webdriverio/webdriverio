    ...
    describe('overwrite native commands', () => {
        it('should allow to overwrite browser commands', async () => {
            ...
            browser.overwriteCommand(
                'getTitle',
                async function (origCommand, pre = '') {
                    return (
                        '' + (await origCommand()).map((result) => pre + result)
                    )
                },
            )

            assert.equal(
                await browser.getTitle('Foo '),
                'Foo Mock Page Title,Foo Mock Page Title',
            )
        })
        ...
        it('should allow to overwrite element commands of a single browser', async () => {
            ...
            global.browserA.overwriteCommand(
                'getTitle',
                async function (origCommand) {
                    return `Title: ${await origCommand()}`
                },
            )
            assert.equal(
                await global.browserA.getTitle(),
                'Title: Mock Page Title',
            )
        ...
        it('should allow to overwrite element commands', async () => {
            ...
            browser.overwriteCommand(
                'getSize',
                async function (origCommand, ratio = 1) {
        ...
        it('should keep the scope', async () => {
            ...
            browser.overwriteCommand(
                'saveRecordingScreen',
                async function (origCommand, filepath, elem) {
                    if (elem) {
                        return (
                            (await this.execute('1+1')) +
                            '-' +
                            elem.instances.map((i) => elem[i].selector)
                        )
                    }
            ...
            assert.equal(
                await browser.saveRecordingScreen(null, await $('body')),
                '2,2-body,body',
            )
        })
        ...
        it('should respect promises - browser', async () => {
            ...
            browser.overwriteCommand(
                'getUrl',
                async function (origCommand, append = '') {
                    return Promise.resolve(
                        (await origCommand()).map((result) => result + append),
                    )
                },
            )

            assert.equal(
                await browser.getUrl('/foobar'),
                'https://mymockpage.com/foobar,https://mymockpage.com/foobar',
            )
        })
